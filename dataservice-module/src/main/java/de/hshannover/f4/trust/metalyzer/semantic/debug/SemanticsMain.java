 ...
 * #L%
 */
package de.hshannover.f4.trust.metalyzer.semantic.debug;
...
public class SemanticsMain implements Runnable {
	...
	public void runSemantics() {
			...
			while (iter.hasNext()) {
				...
				for (Link idLink : id.getLinks()) {
					...
					for (Metadata idMeta : idLink.getMetadata()) {
						log.info("    Metadata from Link: "
								+ idMeta.getTypeName());
					}
					log.info("    Link from: "
							+ idPair.getFirst().getTypeName());
					log.info("           to: "
							+ idPair.getSecond().getTypeName());
				}
		...
		if (metaList != null) {
			...
			while (iter.hasNext()) {
				...
				log.info("Metadata (" + meta.getTypeName()
	...
	private void runCount() {
		...
		MetalyzerAPI api = MetalyzerAPI
				.getInstance(MetalyzerAPI.DEFAULT_CONNECTION);

		log.info("Current Nodes: " + api.getGraphAnalyzer().countNodes());
		log.info("Nodes at 1400845125000: "
				+ api.getGraphAnalyzer().countNodesAt(1401450107000L));
		log.info("Nodes ( 1400845125000, 1400845127000 ): "
				+ api.getGraphAnalyzer().countNodes(1401450107000L,
						1401450113000L));

		log.info("Current Edges: " + api.getGraphAnalyzer().countEdges());
		log.info("Edges at 1400845125000: "
				+ api.getGraphAnalyzer().countEdgesAt(1401450107000L));
		log.info("Edges ( 1400845125000, 1400845127000 ): "
				+ api.getGraphAnalyzer().countEdges(1401450107000L,
						1401450113000L));

		log.info("Current Metadata: " + api.getMetadataFinder().count());
		log.info("Metadata at 1400845125000: "
				+ api.getMetadataFinder().count(1401450107000L));
		log.info("Metadata ( 1400845125000, 1400845127000 ): "
				+ api.getMetadataFinder().count(1401450107000L, 1401450113000L));

		log.info("Mean of current Edges: "
				+ api.getGraphAnalyzer().getMeanOfEdges());
		log.info("Mean of current Edges at 1400845125000: "
				+ api.getGraphAnalyzer().getMeanOfEdges(1401450107000L));
	}
