...
import de.hshannover.f4.trust.metalyzer.semantic.helper.DeviceHelper;
 ...
 * 
 *         Service-class to provide query-methods to search information for
 *         specific MAC-Addresses. The MacAddressService provides methods for
 *         timestamp-, timeinterval- and current-requests.
 * 
 */
public class MacAddressService {
	// SemanticsController for the connection to MacAddress-REST-API and
	// MetalyzerAPI:
	 ...
	 * @return Returns a list of all MAC-Addresses at the given Unix-Timestamp.
	 */
	public ArrayList<MacAddress> getMacAddresses(RequestType requestType,
		...
		// Searching for the MAC-Address-identifiers (timestamp, timeinterval,
		// current).
		...
		for (Identifier macAddressIdentifier : macAddressList) {
			...
			for (Link macAddressLink : macAddressIdentifier.getLinks()) {
				for (Metadata linkMetadata : macAddressLink.getMetadata()) {
					// Searching for the corresponding publish-timestamp of the
					// MAC-Address.
	...
	/**
	 * Query-method to get the publish-timestamp of a specific
	 * MAC-Address-Object.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 ...
	 * @return Returns the Unix-Timestamp of the given MAC-Address.
	 */
	public long getTimestamp(RequestType requestType, MacAddress macAddress)
		...
		Identifier macAddressIdentifier = getMacAddressIdentifier(requestType,
				macAddress);
		...
		if (accessRequestMac != null) {
			// First try to get the publish-timestamp from
	...
	/**
	 * Query-method to get all associated users of a given MAC-Address-Object at
	 * a given Unix-Timestamp. The timestamp-attribute of the MAC-Address-Object
	 * can be chosen freely to narrow down the search-results. In the
	 * IF-MAP-Graph only users with an earlier or same publish-timestamp as the
	 * timestamp-attribute of the given MAC-Address-Object will be found.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param macAddress
	 *            MAC-Address to which the users should be searched.
	 * @return Returns a list of all users of the given MAC-Address.
	 */
	public ArrayList<User> getUsers(RequestType requestType,
			MacAddress macAddress) throws MetalyzerAPIException {
	...
	/**
	 * Query-method to get all associated devices of a given MAC-Address-Object
	 * at a given Unix-Timestamp. The timestamp-attribute of the
	 * MAC-Address-Object can be chosen freely to narrow down the
	 * search-results. In the IF-MAP-Graph only devices with an earlier or same
	 * publish-timestamp as the timestamp-attribute of the given
	 ...
	 * @return Returns a list of all devices of the given MAC-Address.
	 */
		...
		if (macAddressIdentifier != null) {
			...
			for (Link macAddressLink : macAddressIdentifier.getLinks()) {
				...
				if (first.getTypeName().equals("access-request")) {
					...
					// //To find also infrastructure-devices (Switch etc.)
	...
	/**
	 * Query-method to get all associated IP-Addresses of a given
	 * MAC-Address-Object at a given Unix-Timestamp. The timestamp-attribute of
	 * the MAC-Address-Object can be chosen freely to narrow down the
	 * search-results. In the IF-MAP-Graph only IP-Addresses with an earlier or
	 * same publish-timestamp as the timestamp-attribute of the given
	 * MAC-Address-Object will be found.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 ...
	 * @return Returns a list of all IP-Addresses of the given MAC-Address.
	 */
		...
		if (macAddressIdentifier != null) {
			...
			for (Link macAddressLink : macAddressIdentifier.getLinks()) {
				...
				if (first.getTypeName().equals("access-request")) {
					...
					IpAddressHelper.findDirectlyLinkedIpAddresses(first,
							ipAddresses);

					// Second identifier of the link is the IP-Address.
				} else if (second.getTypeName().equals("ip-address")) {
					IpAddressHelper.findDirectlyLinkedIpAddresses(second,
							ipAddresses);
				}
			}
		}
	...
	/**
	 * Helper-method to get the corresponding Identifier of a given
	 * MAC-Address-Object. The timestamp-attributes of the MAC-Address-Object
	 * can be chosen freely to get the identifier in a specific situation of the
	 * IF-MAP-Graph.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param macAddress
	 *            MAC-Address to which the corresponding identifier should be
	 *            searched.
	 * @return Returns the identifier of the given MAC-Address.
	 */
	private Identifier getMacAddressIdentifier(RequestType requestType,
			MacAddress macAddress) throws MetalyzerAPIException {
		// Searching for all MAC-Address-identifiers.
		Collection<Identifier> macAddressList = null;
		if (requestType == RequestType.TIMESTAMP_REQUEST) {
			macAddressList = getAllMacAddressIdentifiers(macAddress
					.getSearchTimestamp());
		} else if (requestType == RequestType.TIMEINTERVAL_REQUEST) {
			macAddressList = getAllMacAddressIdentifiers(
					macAddress.getSearchFromTimestamp(),
					macAddress.getSearchToTimestamp());
		} else if (requestType == RequestType.CURRENT_REQUEST) {
			macAddressList = getAllMacAddressIdentifiers();
		}
		...
		while (macAddressIterator.hasNext() && !foundMacAddress) {
			...
			if (macAddressIdentifier.getTypeName().equals("mac-address")) {
				if (macAddressIdentifier.valueFor("/mac-address[@value]")
						.equals(macAddress.getAddress())) {
	...
	/**
	 * Helper-method to get a collection of all MAC-Address-Identifiers at a
	 * given Unix-Timestamp.
	 * 
	 * @param timestamp
	 *            Unix-Timestamp to which the MAC-Address-identifiers should be
	 *            searched.
	 * @return Returns a collection of all MAC-Address-identifiers at the given
	 *         Unix-Timestamp.
	 */
	private Collection<Identifier> getAllMacAddressIdentifiers(long timestamp)
			throws MetalyzerAPIException {
		log.info("Request to get all MAC-Address-Identifiers from the MetalyzerAPI at timestamp "
				+ timestamp);

		return semCon.getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.MAC_ADDRESS, timestamp);
	}
	 ...
	 *         period of time.
	 */
	private Collection<Identifier> getAllMacAddressIdentifiers(
			long fromTimestamp, long toTimestamp) throws MetalyzerAPIException {
		log.info("Request to get all MAC-Address-Identifiers from the MetalyzerAPI at interval ["
				+ fromTimestamp + ", " + toTimestamp + "]");

		MetalyzerDelta<Identifier> delta = semCon
				.getAPI()
				.getIdentifierFinder()
				.get(StandardIdentifierType.MAC_ADDRESS, fromTimestamp,
						toTimestamp);
	...
	/**
	 * Helper-method to get a collection of all MAC-Address-Identifiers at the
	 * current timestamp.
	 * 
	 * @return Returns a collection of all MAC-Address-identifiers at the
	 *         current timestamp.
	 */
	private Collection<Identifier> getAllMacAddressIdentifiers()
			throws MetalyzerAPIException {
		...
		return semCon.getAPI().getIdentifierFinder()
				.getCurrent(StandardIdentifierType.MAC_ADDRESS);
	}
}
