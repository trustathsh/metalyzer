...
public class MetadataFinderImpl implements MetadataFinder {
	 ...
	 * @author Johannes Busch
	 * @param graphService
	 *            Used to get data from VisITMeta.
	 */
	...
	/**
	 * Returns a filtered collection of known current Metadata in the IF-MAP
	 * graph.
	 * 
	 * @author Sören Grzanna
	 * @param filter
	 ...
	 */
	@Override
	public Collection<Metadata> getCurrent(String filter)
			throws MetalyzerAPIException {
		Collection<Metadata> metacollection = MetadataFinderHelper
				.getMetaCollection(graphServ.getCurrentGraph());
		if (filter == null) {
	...
	/**
	 * Returns a collection of all known Metadata in the IF-MAP graph at the
	 * given timestamp.
	 * 
	 * @author Sören Grzanna
	 * @param filter
	 ...
	 */
	@Override
	public Collection<Metadata> get(long timestamp)
			throws MetalyzerAPIException {
		return get(null, timestamp);
	}

	/**
	 * Returns a filtered collection of all known Metadata in the IF-MAP graph
	 ...
	 */
	@Override
	public Collection<Metadata> get(String filter, long timestamp)
			throws MetalyzerAPIException {
		MetalyzerAPIHelper.isTimestampNegative(timestamp);
		Collection<Metadata> metacollection = MetadataFinderHelper
				.getMetaCollection(graphServ.getGraphAt(timestamp));
		if (filter == null) {
			return metacollection;
		}
		return MetadataFinderHelper.getFiltered(metacollection, filter);
	...
	/**
	 * Returns an Metalyzer Delta build by the given two timestamps with all
	 * Metadata that were present before from-Timestamp.
	 * 
	 * @author Sören Grzanna
	 * @param from
	 *            Timestamps where the Delta should start.
	 * @param to
	 *            Timestamps where the Delta should end.
	 * @return MetalyzerDelta {@link MetalyzerDelta} The availables are from the
	 *         graph at the from timestamp
	 * @throws {@link MetalyzerAPIException} Exception thrown when filter is
	 *         null, timestamps are negative or from is higher then to
	 */
	@Override
	public MetalyzerDelta<Metadata> get(long from, long to)
			throws MetalyzerAPIException {
	...
	/**
	 * Returns an Metalyzer Delta build by the given two timestamps with all
	 * filtered Metadata that were present before from-Timestamp.
	 * 
	 * @author Sören Grzanna
	 * @param filter
	 *            Filter contains a filter of IF-MAP metadata.
	 * @param from
	 *            Timestamps where the Delta should start.
	 * @param to
	 *            Timestamps where the Delta should end.
	 ...
	 */
	@Override
	public MetalyzerDelta<Metadata> get(String filter, long from, long to)
			throws MetalyzerAPIException {
		...
		Collection<Metadata> deletes = MetadataFinderHelper
				.getMetaCollection(delta.getDeletes());
		Collection<Metadata> updates = MetadataFinderHelper
				.getMetaCollection(delta.getUpdates());

		return new MetalyzerDelta<Metadata>(MetadataFinderHelper.getFiltered(
				deletes, filter), MetadataFinderHelper.getFiltered(updates,
				filter), MetadataFinderHelper.getFiltered(preFrom, filter));
	}
