...
/**
 * Helper-Class which provides MAC-Address-Helper-Methods to extract information
...
public abstract class MacAddressHelper {
	...
	/**
	 * Helper-Method to find MacAddress-Identifiers from other identifiers
	 * (mainly used with access-request-identifiers).
	 * 
	 * @param accessRequest
	 ...
	 */
	public static void findMacAddressesFromIdentifier(Identifier accessRequest,
		...
		for (Link link : accessRequest.getLinks()) {
			...
			for (Metadata meta : link.getMetadata()) {
				if (meta.getTypeName().equals("access-request-mac")
			...
			if (first.getTypeName().equals("mac-address")) {
				MacAddress mac = new MacAddress(timestamp,
						first.valueFor("/mac-address[@value]"));
				...
				MacAddress mac = new MacAddress(timestamp,
						second.valueFor("/mac-address[@value]"));
	...
	/**
	 * Helper method to find directly linked MAC-Addresses (over
	 * ip-mac-metadata).
	 * 
	 * @param macAddress
	 ...
	 *            ArrayList where the MAC-Addresses will be saved in.
	 */
	public static void findDirectlyLinkedMacAddresses(Identifier macAddress,
			ArrayList<MacAddress> macAddresses) {
		...
		// If the MAC-Address is linked with an Access-Request then get the
		// publish-timestamp from the access-request-mac-metadata.
		...
		MacAddress mac = new MacAddress(publishTimestamp,
				macAddress.valueFor("/mac-address[@value]"));
	...
	/**
	 * Helper method to find MAC-Addresses which are linked with IP-Addresses
	 ...
	 *            ArrayList where the MAC-Addresses will be saved in.
	 */
	public static void findMacAddressesOverIpAddresses(
			Identifier accessRequest, ArrayList<MacAddress> macAddresses) {
