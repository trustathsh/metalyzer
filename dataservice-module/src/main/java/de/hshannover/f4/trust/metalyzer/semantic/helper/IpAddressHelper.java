 ...
 * #L%
 */
package de.hshannover.f4.trust.metalyzer.semantic.helper;
...
/**
 * Helper-Class which provides IP-Address-Helper-Methods to extract information
 * out of the IF-MAP-Graph.
 * 
 */
public abstract class IpAddressHelper {
	 ...
	 * (mainly used with access-request- and device-identifiers).
	 * 
	 * @param identifier
	 *            Identifier to start from (should be an access-request or
	 *            device (with IP-Address linked via device-ip)).
	 * @param ipAddresses
	 *            ArrayList where the IP-Addresses will be saved in.
	 */
	public static void findIPAddressesFromIdentifier(Identifier identifier,
		...
		for (Link link : identifier.getLinks()) {
			...
			for (Metadata meta : link.getMetadata()) {
				if (meta.getTypeName().equals("access-request-ip")
						|| meta.getTypeName().equals("device-ip")
						|| meta.getTypeName().equals("ip-mac")) {
			...
			if (first.getTypeName().equals("ip-address")) {
				IpAddress ip = new IpAddress(timestamp,
						first.valueFor("/ip-address[@value]"),
						first.valueFor("/ip-address[@type]"));
				...
				IpAddress ip = new IpAddress(timestamp,
						second.valueFor("/ip-address[@value]"),
						second.valueFor("/ip-address[@type]"));
	 ...
	 *            ArrayList where the IP-Addresses will be saved in.
	 */
	public static void findIPAddressIdentfiersFromIdentifier(
			Identifier identifier, ArrayList<Identifier> ipAddresses) {
	 ...
	 *            ArrayList where the IP-Addresses will be saved in.
	 */
	public static void findDeviceIpAddresses(Identifier accessRequest,
			ArrayList<IpAddress> ipAddresses) {
	...
	/**
	 * Helper method to find directly linked IP-Addresses (over
	 * ip-mac-metadata).
	 * 
	 * @param ipAddress
	 ...
	 *            ArrayList where the IP-Addresses will be saved in.
	 */
	public static void findDirectlyLinkedIpAddresses(Identifier ipAddress,
			ArrayList<IpAddress> ipAddresses) {
		...
		// If the IP-Address is linked with an Access-Request then get the
		// publish-timestamp from the access-request-ip-metadata.
		if (accessRequestIp != null) {
			publishTimestamp = accessRequestIp.getPublishTimestamp();

			// If the MAC-Address is not linked with an Access-Request then get
			// the publish-timestamp from the ip-mac-metadata.
		} else if (ipMac != null) {
		...
		IpAddress ip = new IpAddress(publishTimestamp,
				ipAddress.valueFor("/ip-address[@value]"),
				ipAddress.valueFor("/ip-address[@type]"));
		if (!ipAddresses.contains(ip)) {
			ipAddresses.add(ip);
		}
	}
	...
	/**
	 * Helper method to find IP-Addresses which are linked with MAC-Addresses
	 * over ip-mac-metadata.
	 * 
	 * @param accessRequest
	 *            Access-Request-Identifier
	 * @param ipAddresses
	 *            ArrayList where the IP-Addresses will be saved in.
	 */
	public static void findIpAddressesOverMacAddresses(
			Identifier accessRequest, ArrayList<IpAddress> ipAddresses) {
