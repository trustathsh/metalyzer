...
public class StatisticController {
	 ...
	 */
	public double evaluateMean(long from, long to,
	 ...
	 * @throws MetalyzerAPIException
	 */
	public double evaluateMean(StandardIdentifierType filter, MeanType type)
			throws MetalyzerAPIException {
	 ...
	 * @throws MetalyzerAPIException
	 */
	public double evaluateMean(long timestamp, StandardIdentifierType filter,
			MeanType type) throws MetalyzerAPIException {
	 ...
	 * @throws MetalyzerAPIException
	 */
	public HashMap<String, Double> evaluateFrequency(long from, long to,
	 ...
	 * @throws MetalyzerAPIException
	 */
	public HashMap<String, Double> evaluateFrequency(FrequencyType type,
			FrequencySelection selection) throws MetalyzerAPIException {
	 ...
	 * @throws MetalyzerAPIException
	 */
	public HashMap<String, Double> evaluateFrequency(long timestamp,
			FrequencyType type, FrequencySelection selection)
			throws MetalyzerAPIException {
	 ...
	 * @param to
	 *            end time
	 * @return returns the result of the node query as a
	 *         ResultObject<NodeResult>
	 */
	public ResultObject<NodeResult> getNodeCharecteristic(long from, long to) {
		ArrayList<Long> intervalTimestamps = getAPI().getTimestampsBetween(
				from, to);
		...
		for (long timestamp : intervalTimestamps) {
			numbersOfIdentifier = getAPI().getGraphAnalyzer().countNodesAt(
					timestamp);
		...
		for (long timestamp : intervalTimestamps) {
			...
			nodeResult.add(new NodeResult(timestamp, deviation,
					numbersOfIdentifier));
		}
		stdDeviation = new RStandardDeviation(evaluationList)
				.getStandardDeviation();
		return new ResultObject<NodeResult>(meanResult, stdDeviation,
				nodeResult);
	 ...
	 * @param to
	 *            end time
	 * @return returns the result of the edge query as a
	 *         ResultObject<EdgeResult>
	 */
	public ResultObject<EdgeResult> getEdgeCharecteristic(long from, long to) {
		...
		long numbersOfEdges;
		for (long timestamp : intervalTimestamps) {
			numbersOfEdges = getAPI().getGraphAnalyzer()
					.countEdgesAt(timestamp);
		...
		for (long timestamp : intervalTimestamps) {
			...
			edgeResult
					.add(new EdgeResult(timestamp, deviation, numbersOfEdges));
		}

		stdDeviation = new RStandardDeviation(evaluationList)
				.getStandardDeviation();
		return new ResultObject<EdgeResult>(meanResult, stdDeviation,
				edgeResult);
	}
	 ...
	 * @param to
	 *            end time
	 * @return returns the result of the edge per nodes query as a
	 *         ResultObject<MeanOfEdgeResult>
	 */
	public ResultObject<MeanOfEdgeResult> getMeanOfEdgeCharecteristic(
		...
		for (long timestamp : intervalTimestamps) {
			...
			edgeResult.add(new MeanOfEdgeResult(timestamp, deviation,
					meanOfEdges));
		}

		stdDeviation = new RStandardDeviation(edgesEvaluation)
				.getStandardDeviation();
		return new ResultObject<MeanOfEdgeResult>(meanResult, stdDeviation,
				edgeResult);
	}
	 ...
	 *         difference of count of identifier to previous and next timestamp
	 */
	public ArrayList<RateOfChangeResult> rateOfChange(long from, long to,
			StandardIdentifierType type) {
		...
		int current = getAPI().getIdentifierFinder().get(type, currentStamp)
				.size();
		int next = getAPI().getIdentifierFinder().get(type, nextStamp).size();

		resultList.add(new RateOfChangeResult(currentStamp, current, 0, current
				- next));
		...
		for (int i = 1; i < (timestamps.size() - 1); i++) {
			...
			int countOfMetdataPrevious = getAPI().getIdentifierFinder()
					.get(type, previousTimestamp).size();
			int countOfMetadataCurrent = getAPI().getIdentifierFinder()
					.get(type, currentTimestamp).size();
			int countOfMetadataNext = getAPI().getIdentifierFinder()
					.get(type, nextTimestamp).size();

			int differencePrevious = countOfMetadataCurrent
					- countOfMetdataPrevious;
			int differenceNext = countOfMetadataCurrent - countOfMetadataNext;

			RateOfChangeResult resObject = new RateOfChangeResult(
					currentTimestamp, countOfMetadataCurrent,
					differencePrevious, differenceNext);
		...
		int previous = getAPI().getIdentifierFinder().get(type, previousStamp)
				.size();

		resultList.add(new RateOfChangeResult(currentStamp, current, current
				- previous, 0));
	...
	public ArrayList<RateOfChangeResult> rateOfChange(long from, long to,
		...
		int current = getAPI().getMetadataFinder().get(type, currentStamp)
				.size();
		...
		for (int i = 1; i < (timestamps.size() - 1); i++) {
			...
			int countOfMetdataPrevious = getAPI().getMetadataFinder()
					.get(type, previousTimestamp).size();
			int countOfMetadataCurrent = getAPI().getMetadataFinder()
					.get(type, currentTimestamp).size();
			int countOfMetadataNext = getAPI().getMetadataFinder()
					.get(type, nextTimestamp).size();
		...
		int previous = getAPI().getMetadataFinder().get(type, previousStamp)
				.size();
	...
	public ArrayList<RateOfChangeResult> rateOfChange(long from, long to) {
		...
		for (int i = 1; i < (timestamps.size() - 1); i++) {
			...
			int countOfMetdataPrevious = getAPI().getMetadataFinder()
					.get(previousTimestamp).size();
			int countOfMetadataCurrent = getAPI().getMetadataFinder()
					.get(currentTimestamp).size();
			int countOfMetadataNext = getAPI().getMetadataFinder()
					.get(nextTimestamp).size();
	...
	public double relationOfNodesToLinks(long timestamp) {
		double numberOfNodes = getAPI().getGraphAnalyzer().countNodesAt(
				timestamp);
		double numberOfLinks = getAPI().getGraphAnalyzer().countEdgesAt(
				timestamp);
	...
	public int countIpAdresses(long from, long to) throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.IP_ADDRESS, from, to)
				.getAvailables().size();
	}
	 ...
	 * @throws MetalyzerAPIException
	 */
	public int countMacAdresses(long from, long to)
			throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.MAC_ADDRESS, from, to)
				.getAvailables().size();
	}
	...
	public int countDevices(long from, long to) throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.DEVICE, from, to).getAvailables()
				.size();
	}
	...
	public int countIdentity(long from, long to) throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.IDENTITY, from, to).getAvailables()
				.size();
	}
	 ...
	 * @throws MetalyzerAPIException
	 */
	public int countAccessRequests(long from, long to)
			throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.ACCESS_REQUEST, from, to)
				.getAvailables().size();
	}
	...
	public int countIdentifier(long from, long to) throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.ALL, from, to).getAvailables()
				.size();
	...
	public int countIdentifier(long timestamp) throws MetalyzerAPIException {
		return getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.ALL, timestamp).size();
	...
	public int countMetadata(long from, long to) throws MetalyzerAPIException {
		return getAPI().getMetadataFinder().get(from, to).getAvailables()
				.size();
	}
	 ...
	 * @param to
	 * @param filter
	 *            chooses the StandardIdentifier
	 * @param type
	 *            choose the
	 * @return the mean
	 */
	private double calculateMean(long fromOrTimestamp, long to,
			StandardIdentifierType filter, MeanType type) {
		...
		if (isDeltaMethod) {
			listOfIdentifiers = new ArrayList<>(getAPI().getIdentifierFinder()
					.get(filter, fromOrTimestamp, to).getAvailables());
		} else if (fromOrTimestamp != 0.0) {
			listOfIdentifiers = new ArrayList<>(getAPI().getIdentifierFinder()
					.get(filter, fromOrTimestamp));
		} else {
			listOfIdentifiers = new ArrayList<>(getAPI().getIdentifierFinder()
					.getCurrent(filter));
		}
	 ...
	 */
	public HashMap<String, Double> calculateFrequency(long fromOrTimestamp,
			long to, FrequencyType type, FrequencySelection selection) {
		...
		if (isDeltaMethod) {
			if (type == FrequencyType.IDENTIFIER) {
				list = new ArrayList<>(getAPI().getIdentifierFinder()
						.get(StandardIdentifierType.ALL, fromOrTimestamp, to)
						.getAvailables());
			} else if (type == FrequencyType.DEVICES) {
				list = new ArrayList<>(
						getAPI().getIdentifierFinder()
								.get(StandardIdentifierType.DEVICE,
										fromOrTimestamp, to).getAvailables());
			} else if (type == FrequencyType.METADATA) {
				list = new ArrayList<>(getAPI().getMetadataFinder()
						.get(fromOrTimestamp, to).getAvailables());
			} else if (type == FrequencyType.ROLES) {
				list = new ArrayList<>(getAPI().getMetadataFinder()
						.get("role", fromOrTimestamp, to).getAvailables());
			}
		} else if (fromOrTimestamp != 0.0) {
			if (type == FrequencyType.IDENTIFIER) {
				list = new ArrayList<>(getAPI().getIdentifierFinder().get(
						fromOrTimestamp));
			} else if (type == FrequencyType.DEVICES) {
				list = new ArrayList<>(getAPI().getIdentifierFinder().get(
						StandardIdentifierType.DEVICE, fromOrTimestamp));
			} else if (type == FrequencyType.METADATA) {
				list = new ArrayList<>(getAPI().getMetadataFinder().get(
						fromOrTimestamp));
			} else if (type == FrequencyType.ROLES) {
				list = new ArrayList<>(getAPI().getMetadataFinder().get("role",
						fromOrTimestamp));
			}
		} else {
			if (type == FrequencyType.IDENTIFIER) {
				list = new ArrayList<>(getAPI().getIdentifierFinder()
						.getCurrent(StandardIdentifierType.ALL));
			} else if (type == FrequencyType.DEVICES) {
				list = new ArrayList<>(getAPI().getIdentifierFinder()
						.getCurrent(StandardIdentifierType.DEVICE));
			} else if (type == FrequencyType.METADATA) {
				list = new ArrayList<>(getAPI().getMetadataFinder()
						.getCurrent());
			} else if (type == FrequencyType.ROLES) {
