...
/**
 * Helper-Class which provides Device-Helper-Methods to extract information out
 * of the IF-MAP-Graph.
 * 
 */
public abstract class DeviceHelper {
	...
	/**
	 * Helper-Method to find device-identifiers from other identifiers (mainly
	 * used with access-request-identifiers).
	 * 
	 * @param accessRequest
	 ...
	 */
	public static void findDevicesFromIdentifier(Identifier accessRequest,
		...
		for (Link link : accessRequest.getLinks()) {
			...
			// Searching for the publish-timestamp and the attributes of the
			// device.
			long timestamp = 0;
			...
			for (Metadata meta : link.getMetadata()) {
				...
				if (meta.getTypeName().equals("device-attribute")) {
					String attribute = meta
							.valueFor("/meta:device-attribute/name");
			...
			if (first.getTypeName().equals("device")) {
				Device d = new Device(timestamp,
						first.valueFor("/device/name"), attributes);
				...
				Device d = new Device(timestamp,
						second.valueFor("/device/name"), attributes);
	 ...
	 * 
	 * @param identifier
	 *            Identifier to start from (should be an IP-Address or a
	 *            MAC-Address).
	 * @param devices
	 *            ArrayList where the devices will be saved in.
	 */
	public static void findDevicesOverIdentifier(Identifier identifier,
			ArrayList<Device> devices) {
	 ...
	 *            ArrayList where the devices will be saved in.
	 */
	public static void findAccessRequestDevices(Identifier accessRequest,
			ArrayList<Device> devices) {
	 ...
	 *            ArrayList where the devices will be saved in.
	 */
	public static void findAuthenticatedByDevices(Identifier accessRequest,
			ArrayList<Device> devices) {
