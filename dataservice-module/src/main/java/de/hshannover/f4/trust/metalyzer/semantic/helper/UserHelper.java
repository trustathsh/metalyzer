 ...
 * 
 *         Helper-Class which provides User-Helper-Methods to extract
 *         information out of the IF-MAP-Graph.
 * 
 */
public abstract class UserHelper {
	...
	/**
	 * Helper-Method to find user-identifiers from other identifiers (mainly
	 ...
	 */
	public static void findUsersFromIdentifier(Identifier accessRequest,
		...
		for (Link link : accessRequest.getLinks()) {
			...
			if (first.getTypeName().equals("identity")) {
				...
				User user = new User(timestamp,
						first.valueFor("/identity[@name]"), role);
				...
				User user = new User(timestamp,
						second.valueFor("/identity[@name]"), role);
	 ...
	 *            ArrayList where the users will be saved in.
	 */
	public static void findUsersOverIdentifier(Identifier identifier,
			ArrayList<User> users) {
