...
import de.hshannover.f4.trust.metalyzer.semantic.helper.IpAddressHelper;
import de.hshannover.f4.trust.metalyzer.semantic.helper.MacAddressHelper;
import de.hshannover.f4.trust.metalyzer.semantic.helper.UserHelper;
 ...
 * Devices. The DeviceService provides methods for timestamp-, timeinterval- and
 * current-requests.
 * 
...
public class DeviceService {
	...
	/**
	 * Constructor to set a given SemanticsController (mainly used for
	 * Unittests)
	 * 
	 * @param semCon
	 ...
	 * @return Returns all devices available at the given Unix-Timestamp
	 */
	public ArrayList<Device> getDevices(RequestType requestType,
		...
		if (requestType == RequestType.TIMESTAMP_REQUEST) {
			...
			log.info("    searching for Devices in intervall: ["
		...
		for (Identifier deviceId : deviceList) {
			...
			for (Link link : deviceId.getLinks()) {
				// search for publish timestamp of the device and search for
				// attributes
				// connected to it
				for (Metadata meta : link.getMetadata()) {
					if (meta.getTypeName().equals("device-attribute")) {
					...
					} else if (meta.getTypeName().equals(
						...
						log.info("    found timestamp: " + publishTimestamp
								+ " for " + meta.getTypeName() + " device");
					}
				}
			}
			Device device = null;
			...
			if (attributes.size() > 0) {
				device = new Device(publishTimestamp,
						deviceId.valueFor("/device/name"), attributes);
			} else {
				device = new Device(publishTimestamp,
						deviceId.valueFor("/device/name"), attributes);
			}
	 ...
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @return Returns the Unix-Timestamp of the given device.
	 */
	public long getTimestamp(RequestType requestType, Device d) {
		...
		log.info("---- DeviceService: getTimestamp End: found timestamp: "
				+ publishTimestamp + "----");
	 ...
	 * for a given device. Searching for directly linked IP-Addresses via
	 * device-ip. Otherwise searching for access-request-ip or IP-Address linked
	 * to a MAC-Address linked via access-request-mac.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param d
	 * @link{Device to which the Unix-Timestamp should be searched.
	 * @return Returns a list of all IP-Addresses of the given device.
	 */
	public ArrayList<IpAddress> getIpAddresses(RequestType requestType, Device d) {
		...
		boolean found = false;
		if (device != null) {
			log.info("    searching for IP-Addresses for device identifier "
					+ device.valueFor("/device/name"));
			...
			for (Link deviceLink : device.getLinks()) {
				...
				if (first.getTypeName().equals("ip-address")
					...
					IpAddressHelper.findIPAddressesFromIdentifier(device,
							deviceIpAdresses);
					found = true;
					break;
				} else if (first.getTypeName().equals("access-request")
						|| second.getTypeName().equals("access-request")) {
					accessRequestLink = deviceLink;
				}
			}

			// if no directly linked IP-Address is found, search for the
			// access-request-ips
			if (!found && (accessRequestLink != null)) {
				log.info("AccessRequestLink");
				log.info("Found: " + found);
				for (Metadata arMeta : accessRequestLink.getMetadata()) {
					Identifier first = accessRequestLink.getIdentifiers()
							.getFirst();
					Identifier second = accessRequestLink.getIdentifiers()
							.getSecond();
					if (arMeta.getTypeName().equals("access-request-device")) {
						...
						log.info("first Name: "
								+ first.valueFor("/device/name"));
						log.info("first Name: "
								+ first.valueFor("/access-request[@name]"));
						log.info("second: " + second.getTypeName());
						log.info("second Name: "
								+ second.valueFor("/device/name"));
						log.info("second Name: "
								+ second.valueFor("/access-request[@name]"));
						if (first.getTypeName().equals("access-request")) {
							IpAddressHelper.findIPAddressesFromIdentifier(
									first, deviceIpAdresses);
							IpAddressHelper.findIpAddressesOverMacAddresses(
									first, deviceIpAdresses);
							break;
						} else if (second.getTypeName()
								.equals("access-request")) {
							IpAddressHelper.findIPAddressesFromIdentifier(
									second, deviceIpAdresses);
							IpAddressHelper.findIpAddressesOverMacAddresses(
									second, deviceIpAdresses);
							break;
						}
	 ...
	 * for a given device. Searching for directly linked MAC-Addresses linked
	 * via ip-mac from a device-ip IP-Address Identifier. Otherwise searching
	 * for access-request-mac or MAC-Address linked to a IP-Address linked via
	 * access-request-ip.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param d
	 * @link{Device to which the Unix-Timestamp should be searched.
	 * @return Returns a list of all MAC-Addresses of the given device.
	 */
	public ArrayList<MacAddress> getMacAddresses(RequestType requestType,
		...
		Identifier device = getDeviceIdentifier(requestType, d);
		if (device != null) {
			log.info("    searching for MAC-Address for device identifier "
					+ device.valueFor("/device/name"));
			...
			for (Identifier ipId : ipIds) {
				...
				for (Link ipLink : ipId.getLinks()) {
					...
					if (first.getTypeName().equals("mac-address")) {
						...
						MacAddressHelper.findDirectlyLinkedMacAddresses(first,
								deviceMacAdresses);
						found = true;
						break;
					} else if (second.getTypeName().equals("mac-address")) {
						log.info("    second is the Identifier");
						MacAddressHelper.findDirectlyLinkedMacAddresses(second,
								deviceMacAdresses);
						found = true;
						break;
					}
			...
			// if no directly linked MAC-Address is found search from the
			// access-request
			if (!found) {
				...
				for (Link deviceLink : device.getLinks()) {
					...
					for (Metadata deviceMeta : deviceLink.getMetadata()) {
						if (deviceMeta.getTypeName().equals(
								"access-request-device")) {
					...
					if (isArDevice) {
						...
						if (first.getTypeName().equals("access-request")) {
							MacAddressHelper.findMacAddressesFromIdentifier(
									first, deviceMacAdresses);
							MacAddressHelper.findMacAddressesOverIpAddresses(
									first, deviceMacAdresses);
						} else if (second.getTypeName()
								.equals("access-request")) {
							MacAddressHelper.findMacAddressesFromIdentifier(
									second, deviceMacAdresses);
							MacAddressHelper.findMacAddressesOverIpAddresses(
									second, deviceMacAdresses);
						}
					}
	...
	/**
	 * Query-method to get all Users available at given Unix-Timestamp for a
	 * given device. Searching for the users linked to the access-request via
	 * authenticated-as
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param d
	 * @link{Device to which the Unix-Timestamp should be searched.
	 * @return Returns a list of all users of the given device.
	 */
	public ArrayList<User> getUsers(RequestType requestType, Device d) {
		...
		Identifier device = getDeviceIdentifier(requestType, d);
		if (device != null) {
			log.info("    using device identifier: "
					+ device.valueFor("/device/name"));

			// search for all Users linked via authenticated-as to an
			// access-request
			for (Link deviceLink : device.getLinks()) {
	 ...
	 * 
	 * @param d
	 * @link{Identifier from which the IP Identifiers should be searched. Should
	 *                  be of type Device.
	...
	private ArrayList<Identifier> getIPIdentfiers(Identifier d) {
		...
		log.info("    searching for IPAddress identifier for specified identifier "
				+ d.getTypeName());
	...
	/**
	 * Helper method to get the device @link{Identifier} for a given
	 * 
	 * @link{Device object Returns the @link{Identifier} which the Metalyzer
	 *              Semantics Device object represents in the graph
	 * 
	 * @param requestType
	 *            Type of the request
	 * @param d
	 * @link{Device to search the identifier for
	 * @return Device @link{Identifier} which the Metalyzer Semantics
	 * @link{Device object does represent in the graph
	 */
	private Identifier getDeviceIdentifier(RequestType requestType, Device d) {
		...
		if (requestType == RequestType.TIMESTAMP_REQUEST) {
			...
			deviceList = getAllDeviceIdentifiers(d.getSearchFromTimestamp(),
					d.getSearchToTimestamp());
		} else if (requestType == RequestType.CURRENT_REQUEST) {
			deviceList = getAllDeviceIdentifiers();
		}
		if (deviceList != null) {
	 ...
	 */
	private Collection<Identifier> getAllDeviceIdentifiers()
	...
	private Collection<Identifier> getAllDeviceIdentifiers(long fromTimestamp,
		...
		MetalyzerDelta<Identifier> deviceDelta = semCon.getAPI()
				.getIdentifierFinder()
				.get(StandardIdentifierType.DEVICE, fromTimestamp, toTimestamp);
	 ...
	 * @return @link{Collection} of device @link{Identifiers}s
	 */
	private Collection<Identifier> getAllDeviceIdentifiers(long timestamp)
			throws MetalyzerAPIException {
		return semCon.getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.DEVICE, timestamp);
	}
