 ...
 *         specific Ip-Addresses. The IpAddressService provides methods for
...
public class IpAddressService {
	 ...
	 * @return Returns a list of all IP-Addresses at the given Unix-Timestamp.
	 */
	 ...
	 * @return Returns the Unix-Timestamp of the given IP-Address.
	 */
	public long getTimestamp(RequestType requestType, IpAddress ip)
	...
	/**
	 * Query-method to get all associated users of a given IP-Address-Object at
	 * a given Unix-Timestamp. The timestamp-attribute of the IP-Address-Object
	 ...
	 * timestamp-attribute of the given IP-Address-Object will be found.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param ip
	 *            IP-Address to which the users should be searched.
	 * @return Returns a list of all users of the given IP-Address.
	 */
	public ArrayList<User> getUsers(RequestType requestType, IpAddress ip)
			throws MetalyzerAPIException {
	...
	/**
	 * Query-method to get all associated MAC-Addresses of a given
	 * IP-Address-Object at a given Unix-Timestamp. The timestamp-attribute of
	 * the IP-Address-Object can be chosen freely to narrow down the
	 * search-results. In the IF-MAP-Graph only MAC-Addresses with an earlier or
	 ...
	 * @return Returns a list of all MAC-Addresses of the given IP-Address.
	 */
	public ArrayList<MacAddress> getMacAddresses(RequestType requestType,
			IpAddress ip) throws MetalyzerAPIException {
		...
		if (ipAddressIdentifier != null) {
			...
			for (Link ipAddressLink : ipAddressIdentifier.getLinks()) {
				...
				// First identifier of the link is the access-request.
				if (first.getTypeName().equals("access-request")) {
					// Using the FinderHelper-class to find all associated
					// MAC-Addresses.
	...
	/**
	 * Query-method to get all associated devices of a given IP-Address-Object
	 * at a given Unix-Timestamp. The timestamp-attribute of the
	 * IP-Address-Object can be chosen freely to narrow down the search-results.
	 * In the IF-MAP-Graph only devices with an earlier or same
	 ...
	 * @return Returns a list of all devices of the given IP-Address.
	 */
	public ArrayList<Device> getDevices(RequestType requestType, IpAddress ip)
			throws MetalyzerAPIException {
		...
		if (ipAddressIdentifier != null) {
			...
			for (Link ipAddressLink : ipAddressIdentifier.getLinks()) {
				...
				} else if (first.getTypeName().equals("device")
						|| second.getTypeName().equals("device")) {
					DeviceHelper.findDevicesFromIdentifier(ipAddressIdentifier,
							devices);
				}
			}
		}
	 ...
	 * IP-Address-Object. The timestamp-attributes of the IP-Address-Object can
	 * be chosen freely to get the identifier in a specific situation of the
	 * IF-MAP-Graph.
	 * 
	 * @param requestType
	 *            Type of the request (timestamp, interval, current).
	 * @param ip
	 *            IP-Address to which the corresponding identifier should be
	 *            searched.
	 * @return Returns the identifier of the given IP-Address.
	 */
	private Identifier getIpAddressIdentifier(RequestType requestType,
			IpAddress ip) throws MetalyzerAPIException {
		...
		if (requestType == RequestType.TIMESTAMP_REQUEST) {
			...
			ipAddressList = getAllIpAddressIdentifiers(
					ip.getSearchFromTimestamp(), ip.getSearchToTimestamp());
		} else if (requestType == RequestType.CURRENT_REQUEST) {
			ipAddressList = getAllIpAddressIdentifiers();
		}
		...
		while (ipAddressIterator.hasNext() && !foundIpAddress) {
			...
			if (ipAddressIdentifier.getTypeName().equals("ip-address")) {
				if (ipAddressIdentifier.valueFor("/ip-address[@value]").equals(
						ip.getAddress())) {
					foundIpAddress = true;
				}
			}
	...
	/**
	 * Helper-method to get a collection of all Ip-Address-Identifiers at a
	 * given Unix-Timestamp.
	 * 
	 * @param timestamp
	 *            Unix-Timestamp to which the Ip-Address-identifiers should be
	 *            searched.
	 * @return Returns a collection of all Ip-Address-identifiers at the given
	 *         Unix-Timestamp.
	 */
	private Collection<Identifier> getAllIpAddressIdentifiers(long timestamp)
			throws MetalyzerAPIException {
		log.info("Request to get all IP-Address-Identifiers from the MetalyzerAPI at timestamp "
				+ timestamp);
		return semCon.getAPI().getIdentifierFinder()
				.get(StandardIdentifierType.IP_ADDRESS, timestamp);
	}
	 ...
	 *         period of time.
	 */
	private Collection<Identifier> getAllIpAddressIdentifiers(
			long fromTimestamp, long toTimestamp) throws MetalyzerAPIException {
		log.info("Request to get all IP-Address-Identifiers from the MetalyzerAPI at interval ["
				+ fromTimestamp + ", " + toTimestamp + "]");
		MetalyzerDelta<Identifier> delta = semCon
				.getAPI()
				.getIdentifierFinder()
				.get(StandardIdentifierType.IP_ADDRESS, fromTimestamp,
						toTimestamp);
	...
	/**
	 * Helper-method to get a collection of all IP-Address-Identifiers at the
	 * current timestamp.
	 * 
	 * @return Returns a collection of all IP-Address-identifiers at the current
	 *         timestamp.
	 */
	private Collection<Identifier> getAllIpAddressIdentifiers()
			throws MetalyzerAPIException {
