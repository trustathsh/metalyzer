...
public class IdentifierFinderImpl implements IdentifierFinder {
	...
	/**
	 * Returns a Collection of all current Identifiers in the IF-MAP graph of
	 * VisITMeta.
	 * 
	 * @author Johannes Busch
	 * @return {@link Collection}
	 * @throws {@link MetalyzerAPIException} Exception thrown when the filer is
	 *         null
	 */
	@Override
	 ...
	 * @author Johannes Busch
	 * @param filter
	 *            {@link StandardIdentifierType} Filter contains a filter of
	 *            IF-MAP identifier-Types.
	 * @return {@link Collection}
	 * @throws {@link MetalyzerAPIException} Exception thrown when the filer is
	 *         null
	 */
	...
	public Collection<Identifier> getCurrent(StandardIdentifierType filter)
		...
		if (StandardIdentifierType.ALL == filter) {
			return getIdentifierCollection(graphServ.getCurrentGraph(),
					new NoIdentifierFilter());
		}

		IdentifierFilter idFilter = new IdentifierFilter(
				FilterHelper.convertSTItoString(filter));
		return getIdentifierCollection(graphServ.getCurrentGraph(), idFilter);
	}

	/**
	 * Returns a Collection of all Identifiers in the IF-MAP graph of VisITMeta
	...
	public Collection<Identifier> get(StandardIdentifierType filter,
			long timestamp) throws MetalyzerAPIException {
		FilterHelper.isFilterNull(filter);
		MetalyzerAPIHelper.isTimestampNegative(timestamp);

		if (StandardIdentifierType.ALL == filter) {
			return getIdentifierCollection(graphServ.getGraphAt(timestamp),
					new NoIdentifierFilter());
		}
	 ...
	 * Identifier that were present before from-Timestamp.
	 * 
	 * @author Johannes Busch
	 * @param from
	...
	public MetalyzerDelta<Identifier> get(StandardIdentifierType filter,
			long from, long to) throws MetalyzerAPIException {
		FilterHelper.isFilterNull(filter);
	 ...
	 * @return
	 */
	private Collection<Identifier> getIdentifierCollection(
