...
import de.hshannover.f4.trust.metalyzer.semantic.services.DeviceService;
...
public class DeviceServiceTest {
	...
	/**
	 * test getIpAddresses() from DeviceService with an authentication Device
	 * (PDP)
	 */
	@Test
	...
	/**
	 * test getMacAddresses() from DeviceService with an authentication Device
	 * (PDP)
	 */
	@Test
	...
	/**
	 * test getDevices() from DeviceService with a non valid timestamp (negative
	 * test)
	 */
	@Test
	...
	/**
	 * test getTimstamp() from DeviceService with a non valid device (negative
	 * test)
	 */
	@Test
	...
	/**
	 * test getIpAddresses() from DeviceService with a non valid device
	 * (negative test)
	 */
	@Test
	...
	/**
	 * test getMacAddresses() from DeviceService with a non valid device
	 * (negative test)
	 */
	@Test
	...
	/**
	 * test getUsers() from DeviceService with a non valid device (negative
	 * test)
	 */
	@Test
	...
	/**
	 * test getIpAddresses() from DeviceService at current timestamp with an
	 * authentication device (PDP)
	 */
	@Test
	...
	/**
	 * test getIpAddresses() from DeviceService at current timestamp with a
	 * regular device (PC)
	 */
	@Test
	...
	/**
	 * test getMacAddresses() from DeviceService at current timestamp with a
	 * authentication device (PDP)
	 */
	@Test
	...
	/**
	 * test getTimestamp() from DeviceService with a non valid device (negative
	 * test)
	 */
	@Test
	...
	private void whenGetUsersIsCalledWithNonExistingDevice() {
		users = deviceService.getUsers(RequestType.TIMESTAMP_REQUEST,
				nonExistingDevice);
	}

	private void thenUsersWithNonExistingDeviceShouldBe() {
		assertEquals("Tests if the returned list of users is correct.", 0,
				users.size());
	}

	private void whenGetMacAddressesIsCalledWithNonExistingDevice() {
		macAddresses = deviceService.getMacAddresses(
				RequestType.TIMESTAMP_REQUEST, nonExistingDevice);
	}

	private void thenMacAddressesWithNonExistingDeviceShouldBe() {
		assertEquals("Tests if the returned list of MAC-Addresses is correct.",
				0, macAddresses.size());
	}

	private void whenGetIpAddressesIsCalledWithNonExistingDevice() {
		ipAddresses = deviceService.getIpAddresses(
				RequestType.TIMESTAMP_REQUEST, nonExistingDevice);
	}

	private void thenIpAddressesWithNonExistingDeviceShouldBe() {
		assertEquals("Tests if the returned list of IP-Addresses is correct.",
				0, ipAddresses.size());
	}

	private void whenGetTimestampIsCalledWithNonExistingDevice() {
		timestamp = deviceService.getTimestamp(RequestType.TIMESTAMP_REQUEST,
				nonExistingDevice);
	}
	...
	private void whenGetDevicesIsCalledWithNonValidTimestamp() {
		devices = deviceService.getDevices(RequestType.TIMESTAMP_REQUEST,
				nonValidTimestamp, 0);
	}

	private void thenDevicesWithNonValidTimestampShouldBe() {
		assertEquals("Tests if the returned list of devices is correct.", 0,
				devices.size());
	}
	...
	private void thenUsersShouldBe() {
		assertEquals("Tests if the returned list of users is correct.",
				expectedUsers, users);
	}

	private void whenGetMacAddressesIsCalledWithDevice() {
		macAddresses = deviceService.getMacAddresses(
				RequestType.TIMESTAMP_REQUEST, device2);
	}

	private void whenGetMacAddressesIsCalledWithPdpDevice() {
		pdpMacAddresses = deviceService.getMacAddresses(
				RequestType.TIMESTAMP_REQUEST, device);
	}
	...
	private void thenPdpMacAddressesShouldBe() {
		assertEquals("Tests if the returned list of MAC-Addresses is correct.",
				expectedPdpMacAddresses, pdpMacAddresses);
	}
	...
	private void thenAuthDeviceIpAddressesShouldBe() {
		assertEquals("Tests if the returned list of IP-Addresses is correct.",
				expectedAuthIpAddresses, ipAddresses);
	}
	...
	private void thenPCDeviceIpAddressesShouldBe() {
		assertEquals("Tests if the returned list of IP-Addresses is correct.",
				expectedPCIpAddresses, ipAddresses);
	}
	...
	private void thenDevicesShouldBe() {
		assertEquals("Tests if the returned list of devices is correct.",
	...
	private void whenGetCurrentUsersIsCalledWithNonExistingDevice() {
		users = deviceService.getUsers(RequestType.CURRENT_REQUEST,
				nonExistingDevice);
	}
	...
	private void whenGetCurrentMacAddressesIsCalledWithNonExistingDevice() {
		macAddresses = deviceService.getMacAddresses(
				RequestType.CURRENT_REQUEST, nonExistingDevice);
	}
	...
	private void whenGetCurrentIpAddressesIsCalledWithNonExistingDevice() {
		ipAddresses = deviceService.getIpAddresses(RequestType.CURRENT_REQUEST,
				nonExistingDevice);
	}
	...
	private void whenGetCurrentTimestampIsCalledWithNonExistingDevice() {
		timestamp = deviceService.getTimestamp(RequestType.CURRENT_REQUEST,
				nonExistingDevice);
	}
	...
	private void whenGetCurrentMacAddressesIsCalledWithDevice() {
		macAddresses = deviceService.getMacAddresses(
				RequestType.CURRENT_REQUEST, device2);
	}

	private void whenGetCurrentMacAddressesIsCalledWithPdpDevice() {
		pdpMacAddresses = deviceService.getMacAddresses(
				RequestType.CURRENT_REQUEST, device);
	}
	...
	private void whenGetCurrentDevicesIsCalled() {
		devices = deviceService.getDevices(RequestType.CURRENT_REQUEST,
				expectedTimestamp, 0);
	}
	...
	private void givenIFMAPGraphWithDevices() {
		...
		pdpDeviceIpId.addProperty("/ip-address[@value]",
				ipAddress2.getAddress());
		...
		// Metadata
		MetadataImpl authByMeta = new MetadataImpl("authenticated-by", true,
				device.getTimestamp());
		...
		// Metadata
		MetadataImpl devIpMeta = new MetadataImpl("device-ip", true,
				device.getTimestamp());
		...
		// Metadata
		MetadataImpl pdpMacMeta = new MetadataImpl("ip-mac", true,
				pdpMacAddress.getTimestamp());
		...
		// Metadata
		MetadataImpl accReqDevMeta = new MetadataImpl("access-request-device",
				true, device2.getTimestamp());
		MetadataImpl device2AttributeMeta = new MetadataImpl(
				"device-attribute", true, device2.getTimestamp());
		device2AttributeMeta.addProperty("/meta:device-attribute/name",
				deviceAttribute);
		...
		// Metadata
		MetadataImpl accReqIpMeta = new MetadataImpl("access-request-ip", true,
		...
		// Metadata
		MetadataImpl accReqMacMeta = new MetadataImpl("access-request-mac",
				true, macAddress.getTimestamp());
		...
		// Metadata
		MetadataImpl ipMacMeta = new MetadataImpl("ip-mac", true,
				ipAddress.getTimestamp());
		...
		// Metadata
		MetadataImpl authAsMeta = new MetadataImpl("authenticated-as", true,
				user.getTimestamp());
		MetadataImpl roleMeta = new MetadataImpl("role", true,
				user.getTimestamp());
		...
		try {
			Mockito.when(
					identifierFinder.get(StandardIdentifierType.DEVICE,
							device.getTimestamp())).thenReturn(deviceList);
			Mockito.when(
					identifierFinder.getCurrent(StandardIdentifierType.DEVICE))
