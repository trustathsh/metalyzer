...
import de.hshannover.f4.trust.metalyzer.semantic.services.MacAddressService;
...
public class MacAddressServiceTest {
	...
	private MacAddress nonExistingMac = new MacAddress(123456,
			"00:00:00:00:00:00");
	...
	private void whenGetMacAddressesIsCalledWithTimestamp(long expectedTimestamp) {
		macAddresses = macAddressService.getMacAddresses(
	...
	private void whenGetCurrentMacAddressesIsCalled() {
		currentMacAddresses = macAddressService.getMacAddresses(
	...
	private void whenGetTimestampIsCalledWithMacAddress(MacAddress m) {
		timestamp = macAddressService.getTimestamp(
				RequestType.TIMESTAMP_REQUEST, m);
	}
	...
	private void whenGetTimestampCurrentIsCalledWithMacAddress(MacAddress m) {
		timestamp = macAddressService.getTimestamp(RequestType.CURRENT_REQUEST,
				m);
	}
	...
	private void whenGetIpAddressesIsCalledWithMacAddress(MacAddress m) {
		ipAddresses = macAddressService.getIpAddresses(
				RequestType.TIMESTAMP_REQUEST, m);
	}
	...
	private void thenIpAddressesShouldBe() {
		assertEquals(
				"Tests if the returnd ArrayList of ip-addresses is correct.",
				expectedIpAddresses, ipAddresses);
	}

	// getIpAddresses current-request:
	private void whenGetCurrentIpAddressesIsCalledWithMacAddress(MacAddress m) {
		currentIpAddresses = macAddressService.getIpAddresses(
				RequestType.CURRENT_REQUEST, m);
	}
	...
	private void whenGetCurrentUsersIsCalledWithMacAddress(MacAddress m) {
		currentUsers = macAddressService.getUsers(RequestType.CURRENT_REQUEST,
				m);
	}
	...
	private void whenGetDevicesIsCalledWithMacAddress(MacAddress m) {
		devices = macAddressService
				.getDevices(RequestType.TIMESTAMP_REQUEST, m);
	}
	...
	private void whenGetCurrentDevicesIsCalledWithMacAddress(MacAddress m) {
		currentDevices = macAddressService.getDevices(
				RequestType.CURRENT_REQUEST, m);
	}
	...
	// getTimestamp current-request (negative):
	private void whenGetTimestampCurrentIsCalledWithNonExistingMacAddress(
			MacAddress m) {
		timestamp = macAddressService.getTimestamp(RequestType.CURRENT_REQUEST,
				m);
	}
	...
	// getMacAddresses timestamp-request (negative):
	private void whenGetMacAddressesIsCalledWithNonValidTimestamp(
	...
	// getDevices current-request (negative):
	private void whenGetCurrentDevicesIsCalledWithNonExistingMacAddress(
			MacAddress m) {
		currentDevices = macAddressService.getDevices(
				RequestType.CURRENT_REQUEST, m);
	}
	...
	// getIpAddresses timestamp-request (negative):
	private void whenGetIpAddressesIsCalledWithNonExistingMacAddress(
			MacAddress m) {
		ipAddresses = macAddressService.getIpAddresses(
				RequestType.TIMESTAMP_REQUEST, m);
	}
	...
	// getIpAddresses current-request (negative):
	private void whenGetCurrentIpAddressesIsCalledWithNonExistingMacAddress(
			MacAddress m) {
		currentIpAddresses = macAddressService.getIpAddresses(
				RequestType.CURRENT_REQUEST, m);
	}
	...
	// getUsers current-request (negative):
	private void whenGetCurrentUsersIsCalledWithNonExistingMacAddress(
			MacAddress m) {
		currentUsers = macAddressService.getUsers(RequestType.CURRENT_REQUEST,
				m);
	}
	...
	private void givenIFMAPGraphWithMacAddresses() {
		...
		MetadataImpl meta1 = new MetadataImpl("access-request-mac", true,
				mac.getTimestamp());
		...
		MetadataImpl meta2 = new MetadataImpl("authenticated-as", true,
				user.getTimestamp());
		...
		MetadataImpl meta4 = new MetadataImpl("access-request-ip", true,
		...
		try {
			Mockito.when(
					identifierFinder.get(StandardIdentifierType.MAC_ADDRESS,
							mac.getTimestamp())).thenReturn(colId);
			Mockito.when(
					identifierFinder
