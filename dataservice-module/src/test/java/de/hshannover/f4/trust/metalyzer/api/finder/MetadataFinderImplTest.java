...
public class MetadataFinderImplTest {
	...
	private void givenEmptyMockService() {
		...
		DeltaImpl delta = new DeltaImpl(new ArrayList<IdentifierGraph>(),
				new ArrayList<IdentifierGraph>());

		Mockito.when(service.getDelta(Mockito.anyLong(), Mockito.anyLong()))
				.thenReturn(delta);
	...
	public void testGetMetadataWithTimestampNegative()
	...
	@Test(expected = NegativeTimestampException.class)
	public void testGetMetadataDeltaWithNegativeTimestamp()
			throws MetalyzerAPIException {
		givenEmptyMockService();
	...
	public void testGetMetadataDeltaWithImpossibleDelta()
			throws MetalyzerAPIException {
		givenEmptyMockService();
	...
	public void testGetMetadataFilteredByTimestampAndFilter()
			throws MetalyzerAPIException {
		givenListOfMetadata();
	...
	private void givenListOfMetadata() {
		...
		// Mockito.when(service.getGraphAt(Mockito.anyLong(),
		// Mockito.any(MetalyzerGraphFilter.class))).thenReturn(graphs);
	...
	private void thenMetadataFilteredListShouldHaveSize() {
		assertEquals(
				"Compares the sizes of Metadata-Lists filtered by timestamp and filter",
				1, metaList.size());
	}
	...
	private IdentifierGraphImpl getGraphAt11() {
		IdentifierImpl identAccessRequest = new IdentifierImpl("access_request");
		identAccessRequest
				.addMetadata(new MetadataImpl("capability", true, 11));
