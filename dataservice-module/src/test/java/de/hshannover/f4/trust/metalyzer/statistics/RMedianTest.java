...
import de.hshannover.f4.trust.metalyzer.statistic.RMedian;

public class RMedianTest extends TestCase {

	private RMedian rMedian; 
	private ArrayList<Integer> testList;
	
	@Test
	public void testRMedianForPosValues() {
	...
	@Test
	public void testRMedianByEmptyList(){
	...
	private void whenRMeanAddPosArrayList(){
		rMedian = new RMedian(testList);
	}
	
	private void thenPosListShouldBeEvaluateRight(){
		assertEquals("Value should be 5.5", 5.5, rMedian.getMedian());
	}
	...
	private void thenResultShouldBeNull(){
		assertEquals("Value should be 0 when list ist empty", 0.0, rMedian.getMedian());
	}
}
