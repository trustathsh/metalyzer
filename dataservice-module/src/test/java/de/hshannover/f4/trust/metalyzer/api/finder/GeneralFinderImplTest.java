...
public class GeneralFinderImplTest {
	...
	/**
	 * Description: Uses a Filter without any identifiers or metadata. Just
	 * checks if a not filtered Result is returned.
	 */
	@Test
	...
	private void thenFilterFinderShouldReturnEmptyArrayList() {
		assertThat("Should return an full ( unfiltered ) ArrayList!",
				actualIdents, is(expectedIdents));
	}

	/**
	 * Description: Uses a filter just with an device at current. Returns the
	 * filtered graph as an identifier list.
	 * 
	...
	private void thenFilterFinderShouldReturnFilteredIdentifierList() {
		assertThat("Should return a filtered list!", actualIdents,
				is(expectedIdents));
		Mockito.verify(graphServ).getCurrentGraph();
	}

	/**
	 * Description: Uses a filter with a device and a identity at current.
	 * Returns the filtered graph as an identifier list.
	 * 
	 * @throws MetalyzerAPIException
	 */
	@Test
	...
	private void whenFilterMethodCalledWithMultipleIdentifierFilter() {
		...
		filter.or(StandardIdentifierType.DEVICE)
				.or(StandardIdentifierType.IDENTITY).getMetalyzerTime()
				.atCurrent();
	...
	private void thenFilteredGetCurrentGraphShouldBeCalled() {
		assertThat("Should return an identifier list with filtered Metadata",
				actualIdents, is(expectedIdents));
	}
	...
	private void whenFilterMethodCalledWithIdentifierFilterAt() {
		...
		filter.or(StandardIdentifierType.DEVICE)
				.or(StandardIdentifierType.ACCESS_REQUEST).getMetalyzerTime()
				.at(10);
	...
	/**
	 * Description: Filters Metadata ( ip-mac, role ) in a Graph at 10. Returns
	 * a identifier List.
	 * 
	 * @throws MetalyzerAPIException
	 */
	...
	private void whenFilterAsMetadataWithEmptyFilterIsCalled() {
		...
		// Order is important because the link is filtered before the second
		// identifier
	...
	private void thenFinderShouldReturnUnfilteredMetadataListAtCurrent() {
		assertThat("Should return an unfiltered Metadata list", actualMetadata,
				is(expectedMetadata));
	}
	...
	private void thenFinderShouldReturnFilteredMetadataListAtCurrent() {
		assertThat("Should return an filtered Metadata list", actualMetadata,
				is(expectedMetadata));
	}
	...
	private void whenAsIdentifierDeltaIsCalledWithEmptyFilter() {
		...
		expectedIdentDelta = new MetalyzerDelta<Identifier>(deletes, updates,
	...
	private void thenFullMetalyzerDeltaShouldBeReturned() {
		assertThat(actualIdentDelta.getDeletes(),
				is(expectedIdentDelta.getDeletes()));
		assertThat(actualIdentDelta.getUpdates(),
				is(expectedIdentDelta.getUpdates()));
	}
	...
	private void whenAsMetadataDeltaWithFilterIsCalled() {
		...
		expectedMetaDelta = new MetalyzerDelta<>(deletes, updates,
	...
	private void thenFinderShouldReturnFilteredMetadataDelta() {
		assertThat(actualMetaDelta.getDeletes(),
				is(expectedMetaDelta.getDeletes()));
		assertThat(actualMetaDelta.getUpdates(),
				is(expectedMetaDelta.getUpdates()));
	}
}
