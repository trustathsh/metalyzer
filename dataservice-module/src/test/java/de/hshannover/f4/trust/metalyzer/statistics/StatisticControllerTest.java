...
public class StatisticControllerTest {
	...
	private IdentifierFinder identfinder = builder
			.getMockedIdentifierFinderAt10().getIdentifierFinder();
	private MetadataFinder metadataFinder = builder
			.getMockedMetadataFinderAt10().getMetadataFinder();
	...
	private void thenMeanOfLinksOfIdentifiersReturns() {
		double expected = new RMean(
				Counter.countLinksFromIdentifiers(identfinder.get(
						StandardIdentifierType.DEVICE, 10))).getMean();
		assertEquals(expected,
				sc.evaluateMean(StandardIdentifierType.DEVICE, MeanType.LFI),
				0.001);
		;
	}
	...
	private void thenEvaluateFrequencyReturnsMapOfIdentifier() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(identifierColl),
				FrequencyType.IDENTIFIER, FrequencySelection.RELATIVE_FREQUENCY)
				.getFrequency();
		assertEquals(expected,
				sc.evaluateFrequency(10, FrequencyType.IDENTIFIER,
						FrequencySelection.RELATIVE_FREQUENCY));
	}

	private void thenEvaluateFrequencyReturnsMapOfMetada() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(metadataColl), FrequencyType.METADATA,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(10, FrequencyType.METADATA,
				FrequencySelection.RELATIVE_FREQUENCY));
	}

	private void thenEvaluateFrequencyReturnsMapOfDevices() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(identifierColl), FrequencyType.DEVICES,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(10, FrequencyType.DEVICES,
				FrequencySelection.RELATIVE_FREQUENCY));
	}

	private void thenEvaluateFrequencyReturnsMapOfRoles() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(metadataColl), FrequencyType.ROLES,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(10, FrequencyType.ROLES,
				FrequencySelection.RELATIVE_FREQUENCY));
	}
	...
	private void thenEvaluateFrequencyReturnsMapOfIdentifierAtCurrent() {
		HashMap<String, Double> expected = new RFrequency(
				...
				FrequencyType.IDENTIFIER, FrequencySelection.RELATIVE_FREQUENCY)
				.getFrequency();
		assertEquals(expected, sc.evaluateFrequency(FrequencyType.IDENTIFIER,
				FrequencySelection.RELATIVE_FREQUENCY));
	}
	...
	private void thenEvaluateFrequencyReturnsMapOfMetadaAtCurrent() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(metadataColl), FrequencyType.METADATA,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(FrequencyType.METADATA,
				FrequencySelection.RELATIVE_FREQUENCY));
	}
	...
	private void thenEvaluateFrequencyReturnsMapOfDevicesAtCurrent() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(identifierColl), FrequencyType.DEVICES,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(FrequencyType.DEVICES,
				FrequencySelection.RELATIVE_FREQUENCY));
	}
	...
	private void thenEvaluateFrequencyReturnsMapOfRolesAtCurrent() {
		HashMap<String, Double> expected = new RFrequency(
				new ArrayList<Propable>(metadataColl), FrequencyType.ROLES,
				FrequencySelection.RELATIVE_FREQUENCY).getFrequency();
		assertEquals(expected, sc.evaluateFrequency(FrequencyType.ROLES,
				FrequencySelection.RELATIVE_FREQUENCY));
	}
