...
import de.hshannover.f4.trust.metalyzer.semantic.services.UserService;
...
public class UserServiceTest {
	...
	private void whenGetUsersIsCalledWithTimestamp(long expectedTimestamp) {
		users = userService.getUsers(RequestType.TIMESTAMP_REQUEST,
				expectedTimestamp, 0);
	}
	...
	// getUsers timestamp-request (negative):
	private void whenGetUsersIsCalledWithNonValidTimestamp(
	...
	private void whenGetIpAddressesIsCalledWithUser(User u) {
		ipAddresses = userService.getIpAddresses(RequestType.TIMESTAMP_REQUEST,
				u);
	}
	...
	private void whenGetCurrentIpAddressesIsCalledWithUser(User u) {
		currentIpAddresses = userService.getIpAddresses(
				RequestType.CURRENT_REQUEST, u);
	}
	...
	private void whenGetMacAddressesIsCalledWithUser(User u) {
		macAddresses = userService.getMacAddresses(
				RequestType.TIMESTAMP_REQUEST, u);
	}
	...
	private void whenGetCurrentMacAddressesIsCalledWithUser(User u) {
		currentMacAddresses = userService.getMacAddresses(
				RequestType.CURRENT_REQUEST, u);
	}
	...
	private void givenIFMAPGraphWithUsers() {
		...
		try {
			Mockito.when(
					identifierFinder.get(StandardIdentifierType.IDENTITY,
							user.getTimestamp())).thenReturn(colId);
			Mockito.when(
					identifierFinder
