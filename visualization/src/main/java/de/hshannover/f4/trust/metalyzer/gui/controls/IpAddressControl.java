...
public class IpAddressControl extends SemanticControl {
	...
	public void fillControl(AnalysePanel panel, long t1, long t2) {
		...
		try {
			...
			case Intervall:
				ipAddresses = mConnection.getAllIpAddressesFromTo(
			...
			if (!characteristicName.equals(CharacteristicLabels.IP_AUTH)) {
				...
				for (Long timestamp : mGuiController.getTimestampList()) {
					if ((lastTimestamp == null)
							|| !mConnection.getAllIpAddresses(timestamp)
									.equals(mConnection
											.getAllIpAddresses(lastTimestamp))) {
						filteredTimestampList.add(timestamp);
						lastTimestamp = timestamp;
					}
				}
				mIpAddressPanel.setTimestamps(UtilHelper
						.convertTimestamps(filteredTimestampList));
				mIpAddressPanel.setTimestampItems(UtilHelper
						.convertTimestamps(filteredTimestampList));
			}
	...
	/**
	 * Decides which ip-characteristic was selected and calls the method to fill
		...
		try {
			...
			case Intervall:
				...
				macAddresses = mConnection
						.getMacAddressesForIpAddressFromTo(ipAddress);
				break;
			case Live:
				macAddresses = mConnection
						.getMacAddressesForIpAddressCurrent(ipAddress);
				break;
			case Timestamp:
				ipAddress.setSearchTimestamp(mTimestampTo);
				macAddresses = mConnection
						.getMacAddressesForIpAddress(ipAddress);
				break;
			default:
				macAddresses = null;
				break;
	...
	private void fillIpAddressMacView(IpAddressPanel ipAddressPanel, String ip) {
		...
		try {
			...
			if (macAddresses != null) {
				...
				ipAddressPanel.setCountOfMacAddresses(numberOfMacAddresses);
				ipAddressPanel.setPropertyData(UtilHelper
	 ...
	 * @param ip
	 */
	private void fillIpAddressDeviceView(IpAddressPanel ipAddressPanel,
			String ip) {
		IpAddress ipAddress = ipAddressPanel.getIp(ip);
		if (ipAddress == null) {
			ipAddressPanel.setCountOfDevices(0);
	 ...
	 * @param datestring
	 */
	private void fillIpAddressAuthenticationsView(
			IpAddressPanel ipAddressPanel, String datestring) {
		// Disables the selection controls, is necessary for the exporter to
		// determine which fields to be filled
		ipAddressPanel.getTimeSelectionView().setSelectionMode(
				TimeSelection.None);
