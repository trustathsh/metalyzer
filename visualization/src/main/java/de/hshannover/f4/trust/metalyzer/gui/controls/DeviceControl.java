...
public class DeviceControl extends SemanticControl {
	...
	public void fillControl(AnalysePanel panel, long t1, long t2) {
		...
		numberOfUsers = 0;
		try {
			ArrayList<Device> devices = mConnection
					.getAllDevices(mGuiController.getLastTimestamp());
			switch (mTimeSelectionView.getSelectionMode()) {
			case Intervall:
				devices = mConnection.getAllDevicesFromTo(mTimestampFrom,
						mTimestampTo);
				break;
			case Live:
			...
			if (!characteristicName.equals(CharacteristicLabels.DEVICE_AUTH)) {
				...
				for (Long timestamp : mGuiController.getTimestampList()) {
					if ((lastTimestamp == null)
							|| !mConnection.getAllDevices(timestamp).equals(
									mConnection.getAllDevices(lastTimestamp))) {
						filteredTimestampList.add(timestamp);
						lastTimestamp = timestamp;
					}
				}
				mDevicePanel.setTimestamps(UtilHelper
						.convertTimestamps(filteredTimestampList));
				mDevicePanel.setTimestampItems(UtilHelper
						.convertTimestamps(filteredTimestampList));
			}
	...
	/**
	 * Decides which device-characteristic was selected and calls the method to
		...
		try {
			...
			case Intervall:
				...
				macAddresses = mConnection
						.getMacAddressesForDeviceFromTo(device);
				break;
			case Live:
				macAddresses = mConnection
						.getMacAddressesForDeviceCurrent(device);
				break;
			case Timestamp:
	...
	private void fillDeviceIpView(DevicePanel devicePanel, String dev) {
		...
		try {
			...
			case Live:
				ipAddresses = mConnection
						.getIpAddressesForDeviceCurrent(device);
				break;
			case Timestamp:
				device.setSearchTimestamp(mTimestampTo);
	...
	private void fillDeviceMacView(DevicePanel devicePanel, String dev) {
		...
		try {
			...
			if (macAddresses != null) {
				...
				devicePanel.setCountOfMacAddresses(numberOfMacAddresses);
				devicePanel.setPropertyData(UtilHelper
