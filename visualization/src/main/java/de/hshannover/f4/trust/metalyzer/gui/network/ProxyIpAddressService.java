...
public class ProxyIpAddressService implements IpAddressService {
		...
		if (status == Status.OK) {
			...
			MessageBox
					.showErrorOutputDialog(
							status.getReasonPhrase(),
							"No ipaddresses for time intervall "
									+ UtilHelper
	...
	public ArrayList<IpAddress> getCurrentIpAddresses()
		...
		if (status == Status.OK) {
			...
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No ipaddresses for the current timestamp were found",
					error);
		}
		return ipaddresses;
	}
	...
	public long getTimestampForIpAddress(IpAddress ip)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No timestamp for ipaddress " + ip.getAddress()
							+ " was found", error);
		}
		return timestamp;
	}
		...
		if (status == Status.OK) {
			...
			MessageBox
					.showErrorOutputDialog(status.getReasonPhrase(),
							"No users for ipaddress " + ip.getAddress()
									+ " were found", error);
		}
		return users;
	}
	...
	public ArrayList<Device> getDevicesForIpAddress(IpAddress ip)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No devices for ipaddress " + ip.getAddress()
							+ " were found", error);
		}
		return devices;
	}
	...
	@Override
	public ArrayList<MacAddress> getMacAddressesForIpAddress(IpAddress ip)
			throws InterruptedException, ExecutionException, TimeoutException {
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No macaddresses for ipaddress " + ip.getAddress()
							+ " were found", error);
		}
		return macAddresses;
	}
