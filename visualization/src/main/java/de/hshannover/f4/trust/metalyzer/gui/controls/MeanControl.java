	...
	public void updateControl(AnalysePanel panel, long t1, long t2) {
		...
		try {
			if (characteristic.equals(CharacteristicLabels.MEAN_LINKS)) {
				...
				case Intervall:
					meanOfAll = mConnection.getMeanFromTo(t1, t2,
							StandardIdentifierType.ALL, MeanType.LFI);
					meanOfIpAddresses = mConnection.getMeanFromTo(t1, t2,
							StandardIdentifierType.IP_ADDRESS, MeanType.LFI);
					meanOfMacAddresses = mConnection.getMeanFromTo(t1, t2,
							StandardIdentifierType.MAC_ADDRESS, MeanType.LFI);
					meanOfDevices = mConnection.getMeanFromTo(t1, t2,
							StandardIdentifierType.DEVICE, MeanType.LFI);
					meanOfAccessRequests = mConnection
							.getMeanFromTo(t1, t2,
									StandardIdentifierType.ACCESS_REQUEST,
									MeanType.LFI);
					meanOfIdentities = mConnection.getMeanFromTo(t1, t2,
							StandardIdentifierType.IDENTITY, MeanType.LFI);
					break;
				case Live:
					meanOfAll = mConnection.getMeanCurrent(
							StandardIdentifierType.ALL, MeanType.LFI);
					meanOfIpAddresses = mConnection.getMeanCurrent(
							StandardIdentifierType.IP_ADDRESS, MeanType.LFI);
					meanOfMacAddresses = mConnection.getMeanCurrent(
							StandardIdentifierType.MAC_ADDRESS, MeanType.LFI);
					meanOfDevices = mConnection.getMeanCurrent(
							StandardIdentifierType.DEVICE, MeanType.LFI);
					meanOfAccessRequests = mConnection
							.getMeanCurrent(
									StandardIdentifierType.ACCESS_REQUEST,
									MeanType.LFI);
					meanOfIdentities = mConnection.getMeanCurrent(
							StandardIdentifierType.IDENTITY, MeanType.LFI);
					break;
				case Timestamp:
					meanOfAll = mConnection.getMeanTimestamp(t2,
							StandardIdentifierType.ALL, MeanType.LFI);
					meanOfIpAddresses = mConnection.getMeanTimestamp(t2,
							StandardIdentifierType.IP_ADDRESS, MeanType.LFI);
					meanOfMacAddresses = mConnection.getMeanTimestamp(t2,
							StandardIdentifierType.MAC_ADDRESS, MeanType.LFI);
					meanOfDevices = mConnection.getMeanTimestamp(t2,
							StandardIdentifierType.DEVICE, MeanType.LFI);
					meanOfAccessRequests = mConnection
							.getMeanTimestamp(t2,
									StandardIdentifierType.ACCESS_REQUEST,
									MeanType.LFI);
					meanOfIdentities = mConnection.getMeanTimestamp(t2,
							StandardIdentifierType.IDENTITY, MeanType.LFI);
					break;
				default:
					break;
