...
public class UserControl extends SemanticControl {
	...
	public void fillControl(AnalysePanel panel, long t1, long t2) {
		...
		try {
			...
			case Intervall:
				users = mConnection.getAllUsersFromTo(mTimestampFrom,
						mTimestampTo);
				break;
			case Live:
			...
			if (!characteristicName.equals(CharacteristicLabels.USER_AUTH)) {
				...
				for (Long timestamp : mGuiController.getTimestampList()) {
					if ((lastTimestamp == null)
							|| !mConnection.getAllUsers(timestamp).equals(
									mConnection.getAllUsers(lastTimestamp))) {
						filteredTimestampList.add(timestamp);
						lastTimestamp = timestamp;
					}
				}
				mUserPanel.setTimestamps(UtilHelper
						.convertTimestamps(filteredTimestampList));
				mUserPanel.setTimestampItems(UtilHelper
						.convertTimestamps(filteredTimestampList));
			}
	...
	/**
	 * Decides which user-characteristic was selected and calls the method to
	...
	private void fillUserAllView(UserPanel userPanel, String username) {
		...
		try {
			...
			if (macAddresses != null) {
				...
				String datestring = UtilHelper
						.convertTimestampToDatestring(firstLogin);
						...
						for (Object element : elementList) {
							...
							for (int i = 0; i < elementListSizes[2]; i++) {
								if (element instanceof MacAddress) {
									macAddr.add(((MacAddress) element)
											.getAddress());
								}
							}
	...
	private void fillMacView(UserPanel userPanel, String username) {
		...
		try {
			...
			if (macAddresses != null) {
				numberOfMacAddresses = macAddresses.size();
				userPanel.setCountOfMacAddresses(numberOfMacAddresses);
				userPanel.setPropertyData(UtilHelper
						.convertMacToString(macAddresses));
			}
		} catch (ExecutionException e) {
			log.error(e.getMessage());
		} catch (TimeoutException e) {
	 ...
	 * @param datestring
	 */
	private void fillAuthenticationsView(UserPanel userPanel, String datestring) {
