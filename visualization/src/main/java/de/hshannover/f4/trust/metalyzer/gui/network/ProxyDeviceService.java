...
public class ProxyDeviceService implements DeviceService {
	...
	public long getTimestampForDevice(Device dev) throws InterruptedException,
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No timestamp for device " + dev.getName() + " was found",
					error);
		}
		return timestamp;
	}
	...
	public ArrayList<IpAddress> getIpAddressesForDevice(Device dev)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No ipaddresses for device " + dev.getName()
							+ " were found", error);
		}
		return ipaddresses;
	}
	...
	public ArrayList<MacAddress> getMacAddressesForDevice(Device dev)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No macaddresses for device " + dev.getName()
							+ " were found", error);
		}
		return macAddresses;
	}
	...
	public ArrayList<User> getUsersForDevice(Device dev)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No users for device " + dev.getName() + " were found",
					error);
		}
		return users;
	}
