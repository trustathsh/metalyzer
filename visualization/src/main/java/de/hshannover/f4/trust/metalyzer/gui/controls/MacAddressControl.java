...
public class MacAddressControl extends SemanticControl {
	...
	public void fillControl(AnalysePanel panel, long t1, long t2) {
		...
		try {
			...
			case Intervall:
				macAddresses = mConnection.getAllMacAddressesFromTo(
			...
			if (!characteristicName.equals(CharacteristicLabels.MAC_AUTH)) {
				...
				for (Long timestamp : mGuiController.getTimestampList()) {
					if ((lastTimestamp == null)
							|| !mConnection.getAllMacAddresses(timestamp)
									.equals(mConnection
											.getAllMacAddresses(lastTimestamp))) {
						filteredTimestampList.add(timestamp);
						lastTimestamp = timestamp;
					}
				}
				mMacAddressPanel.setTimestamps(UtilHelper
						.convertTimestamps(filteredTimestampList));
				mMacAddressPanel.setTimestampItems(UtilHelper
						.convertTimestamps(filteredTimestampList));
			}
	...
	/**
	 * Decides which mac-characteristic was selected and calls the method to
	 * fill the corresponding view
	 * 
	 * @param semanticPanel
	 ...
	 * @param mac
	 */
	private void fillMacAddressAllView(MacAddressPanel mMacAddressPanel,
			String mac) {
		...
		try {
			...
			case Intervall:
				...
				ipAddresses = mConnection
						.getIpAddressesForMacAddressFromTo(macAddress);
				break;
			case Live:
				devices = mConnection
						.getDevicesForMacAddressCurrent(macAddress);
				users = mConnection.getUsersForMacAddressCurrent(macAddress);
				ipAddresses = mConnection
						.getIpAddressesForMacAddressCurrent(macAddress);
				break;
			case Timestamp:
				...
				ipAddresses = mConnection
						.getIpAddressesForMacAddress(macAddress);
				break;
			default:
				devices = null;
				users = null;
			...
			if (macList != null) {
				...
				for (ArrayList<?> elementList : macList) {
					...
					for (Object element : elementList) {
						...
						if (elementList.get(elementList.size() - 1).equals(
								element)
	 ...
	 * @param mac
	 */
	private void fillMacAddressUserView(MacAddressPanel mMacAddressPanel,
			String mac) {
		MacAddress macAddress = mMacAddressPanel.getMac(mac);
		if (macAddress == null) {
			mMacAddressPanel.setCountOfUsers(0);
	 ...
	 * @param mac
	 */
	private void fillMacAddressIpView(MacAddressPanel mMacAddressPanel,
			String mac) {
		MacAddress macAddress = mMacAddressPanel.getMac(mac);
		if (macAddress == null) {
			mMacAddressPanel.setCountOfIpAddresses(0);
		...
		try {
			...
			if (ipAddresses != null) {
				...
				mMacAddressPanel.setCountOfIpAddresses(numberOfIpAddresses);
				List<Collection<?>> data = new ArrayList<Collection<?>>();
				List<String> addr = UtilHelper
						.convertIpAddressToString(ipAddresses);
				List<String> type = UtilHelper
						.convertIpTypeToString(ipAddresses);
				data.add(addr);
				data.add(type);
				mMacAddressPanel.setPropertyData(data);
			}
	 ...
	 * @param mac
	 */
	private void fillMacAddressDeviceView(MacAddressPanel mMacAddressPanel,
			String mac) {
		MacAddress macAddress = mMacAddressPanel.getMac(mac);
		if (macAddress == null) {
			mMacAddressPanel.setCountOfDevices(0);
		...
		try {
			...
			if (devices != null) {
				...
				mMacAddressPanel.setCountOfDevices(numberOfDevices);
				List<Collection<?>> data = new ArrayList<Collection<?>>();
				List<String> dev = UtilHelper
						.convertDeviceNameToString(devices);
				List<String> devattr = UtilHelper
						.convertDeviceAttributesToString(devices);
				data.add(dev);
				data.add(devattr);
				mMacAddressPanel.setPropertyData(data);
			}
	 ...
	 * @param datestring
	 */
	private void fillMacAddressAuthenticationsView(
			MacAddressPanel macAddressPanel, String datestring) {
		// Disables the selection controls, is necessary for the exporter to
		// determine which fields to be filled
		macAddressPanel.getTimeSelectionView().setSelectionMode(
				TimeSelection.None);
