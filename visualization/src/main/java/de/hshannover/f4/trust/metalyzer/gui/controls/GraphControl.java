...
public class GraphControl extends StatisticControl {
	...
	public void updateControl(AnalysePanel panel, long t1, long t2) {
		...
		mGraphPanel.getTimeSelectionView().setSelectionMode(
				TimeSelection.Intervall);

		String characteristic = mGraphPanel.getCharacteristic();
		try {
			if (characteristic.equals(CharacteristicLabels.GRAPH_NODES)) {
				ResultObject<NodeResult> result = mConnection
						.getNodeCharecteristic(t1, t2);
				if (result != null) {
					mGraphPanel.setGraphMean(UtilHelper.getPrecsion(
							result.getMean(), 1, 3));
					mGraphPanel.setGraphStandardDeviation(UtilHelper
							.getPrecsion(result.getStandardDeviation(), 1, 3));
				...
				ResultObject<EdgeResult> result = mConnection
						.getEdgeCharecteristic(t1, t2);
				if (result != null) {
					mGraphPanel.setGraphMean(UtilHelper.getPrecsion(
							result.getMean(), 1, 3));
					mGraphPanel.setGraphStandardDeviation(UtilHelper
				...
				ResultObject<MeanOfEdgeResult> result = mConnection
						.getMeanOfEdgeCharecteristic(t1, t2);
				if (result != null) {
					mGraphPanel.setGraphMean(UtilHelper.getPrecsion(
							result.getMean(), 1, 3));
					mGraphPanel.setGraphStandardDeviation(UtilHelper
	...
	private void fillNodeData(GraphPanel graphPanel,
			ArrayList<NodeResult> resultList) {
		if (graphPanel != null) {
			...
			for (NodeResult nodeResult : resultList) {
				String timestamp = UtilHelper
						.convertTimestampToDatestring(nodeResult.getTimestamp());
				long nodes = nodeResult.getNumbersOfIdentifer();
				graphPanel.addPropertyData(timestamp, nodes,
						nodeResult.getDeviation());
			}
		}
	}

	private void fillEdgeData(GraphPanel graphPanel,
			ArrayList<EdgeResult> resultList) {
		if (graphPanel != null) {
			graphPanel.clearProperties();
			graphPanel.setPropertyPrecsion(2, 3, 3);
			...
			for (EdgeResult edgeResult : resultList) {
				String timestamp = UtilHelper
						.convertTimestampToDatestring(edgeResult.getTimestamp());
				long edges = edgeResult.getNumbersOfEgde();
				graphPanel.addPropertyData(timestamp, edges,
						edgeResult.getDeviation());
			}
		}
	}

	private void fillMeanData(GraphPanel graphPanel,
			ArrayList<MeanOfEdgeResult> resultList) {
		if (graphPanel != null) {
			graphPanel.clearProperties();
			...
			for (MeanOfEdgeResult meanResult : resultList) {
				String timestamp = UtilHelper
						.convertTimestampToDatestring(meanResult.getTimestamp());
				double mean = meanResult.getMeanOfEdges();
				graphPanel.addPropertyData(timestamp, mean,
						meanResult.getDeviation());
			}
		...
		if (characteristic.equals(CharacteristicLabels.GRAPH_NODES)) {
			if (mNodeResultList != null) {
				...
				for (int i = lowerBoundary; i < upperBoundary; i++) {
					...
					String timestamp = UtilHelper
							.convertTimestampToDatestring(nodeResult
									.getTimestamp());
					long nodes = nodeResult.getNumbersOfIdentifer();
			...
			if (mEdgeResultList != null) {
				...
				for (int i = lowerBoundary; i < upperBoundary; i++) {
					...
					String timestamp = UtilHelper
							.convertTimestampToDatestring(edgeResult
									.getTimestamp());
					long edges = edgeResult.getNumbersOfEgde();
			...
			if (mMeanResultList != null) {
				...
				for (int i = lowerBoundary; i < upperBoundary; i++) {
					...
					String timestamp = UtilHelper
							.convertTimestampToDatestring(meanResult
									.getTimestamp());
					double mean = meanResult.getMeanOfEdges();
