...
public class TabChangeHandler implements ChangeListener {
	...
	public TabChangeHandler(MetalyzerGuiController guiController,
			ClosableTabbedPane closeTabPane) {
	...
	/**
	 * Triggers a {@link ChangeEvent}, when a the tab changes. Therefore the
	 * preferences of the previous tab where transfered to the current tab
	 */
	@Override
	public void stateChanged(ChangeEvent e) {
		...
		if (prevTabIndex != curTabIndex && prevTabIndex != -1) {
			CategoryPanel prevSelectionPanel = (CategoryPanel) mCloseTabPane
					.getComponentAt(prevTabIndex);
			AnalysePanel prevAnalysePanel = prevSelectionPanel
					.getAnalysePanel();
			TimeSelectionView prevTimeView = prevAnalysePanel
		...
		if (curTabIndex != -1) {
			CategoryPanel curSelectionPanel = (CategoryPanel) mCloseTabPane
					.getComponentAt(curTabIndex);
			AnalysePanel curAnalysePanel = curSelectionPanel.getAnalysePanel();
			TimeSelectionView curTimeView = curAnalysePanel
					.getTimeSelectionView();
