...
public class AnalysePanel extends JPanel implements Observer {
	...
	public AnalysePanel(MetalyzerGuiController gc, String c) {
		...
		mTimeSelectionView = new TimeSelectionView(
				mGuiController.getTimestampList());
		mTimeSelectionView.setLayoutAlignment(FlowLayout.CENTER);
		mTimeSelectionView
				.setSelectionMode(TimeSelectionView.TimeSelection.Timestamp);
		mTimeSelectionView.addTimeIntervallListener(new TimeIntervallHandler(
				mGuiController, this, mTimeSelectionView));
		mTimeSelectionView.addTimeStampListener(new TimeStampHandler(
				mGuiController, this, mTimeSelectionView));
		mTimeSelectionView.addTimeSelectionListener(new TimeSelectionHandler(
				mGuiController, this));
		...
		mPropertiesView.setTableSize((mGuiController.getViewWidth() / 2) + 100,
				150);
	 ...
	 * 
	 * @param columnIndex
	 *            specifies the column that s be formated
	 * @param format
	 * */
	 ...
	 * 
	 * @param names
	 *            , the amount of names, which can be specified, depends on the
	 ...
	 * 
	 * @param values
	 *            the amount of values per row, which can be specified, depends
	 *            on the current column count.
	 */
