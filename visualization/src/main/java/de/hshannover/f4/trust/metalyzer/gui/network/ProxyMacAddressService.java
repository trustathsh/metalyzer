...
public class ProxyMacAddressService implements MacAddressService {
	...
	public ProxyMacAddressService(AsyncWebResource analyseRessource) {
		this.mAnalyseRessource = analyseRessource.path("semantics").path(
				"macaddress");
		mGson = new Gson();
	}
	...
	public ArrayList<MacAddress> getAllMacAddresses(long timestamp)
		...
		mFuture = mAnalyseRessource.path("macaddresses")
				.path(mGson.toJson(timestamp)).accept(MediaType.TEXT_PLAIN)
		...
		if (status == Status.OK) {
			...
			MessageBox
					.showErrorOutputDialog(
							status.getReasonPhrase(),
							"No macaddresses for time intervall "
									+ UtilHelper
	...
	public ArrayList<MacAddress> getCurrentMacAddresses()
		...
		if (status == Status.OK) {
			...
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No macaddresses for the current timestamp were found",
					error);
		}
		return macaddresses;
	}
	...
	public long getTimestampForMacAddress(MacAddress mac)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No timestamp for macaddress " + mac.getAddress()
							+ " was found", error);
		}
		return timestamp;
	}
	...
	public ArrayList<User> getUsersForMacAddress(MacAddress mac)
		...
		mFuture = mAnalyseRessource.path("users").accept(MediaType.TEXT_PLAIN)
				.put(ClientResponse.class, mGson.toJson(mac));
		mClientResponse = mFuture.get(REQUEST_TIMEOUT, TimeUnit.SECONDS);
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No users for macaddress " + mac.getAddress()
							+ " were found", error);
		}
		return users;
	}
	...
	public ArrayList<User> getUsersForMacAddressFromTo(MacAddress mac)
			throws InterruptedException, ExecutionException, TimeoutException {
		ArrayList<User> users = null;
		mFuture = mAnalyseRessource.path("users").path("fromto")
				.accept(MediaType.TEXT_PLAIN)
				.put(ClientResponse.class, mGson.toJson(mac));
		mClientResponse = mFuture.get(REQUEST_TIMEOUT, TimeUnit.SECONDS);
	...
	public ArrayList<Device> getDevicesForMacAddress(MacAddress mac)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No devices for macaddress " + mac.getAddress()
							+ " were found", error);
		}
		return devices;
	}
	...
	public ArrayList<IpAddress> getIpAddressesForMacAddress(MacAddress mac)
		...
		if (status == Status.OK) {
			...
			String error = mClientResponse.getEntity(String.class);
			MessageBox.showErrorOutputDialog(status.getReasonPhrase(),
					"No ipaddresses for macaddress " + mac.getAddress()
							+ " were found", error);
		}
		return ipaddresses;
	}
