...
public class MetalyzerGuiController extends Observable implements Observer {
	...
	private static final Logger log = Logger
			.getLogger(MetalyzerGuiController.class);
	...
	public MetalyzerGuiController(int viewWidth, int viewHeight,
			AnalyseConnection connection, TimePollService pollService) {
	...
	public MetalyzerGuiController(AnalyseConnection connection,
			TimePollService pollService) {
		this.mConnection = connection;
	...
	private void initControls() {
		ControlManager.registerControl(CategoryLabels.FREQUENCY,
				new FrequencyControl(this));
		ControlManager.registerControl(CategoryLabels.MEAN, new MeanControl(
				this));
		ControlManager.registerControl(CategoryLabels.GRAPH, new GraphControl(
				this));
		ControlManager.registerControl(CategoryLabels.USER, new UserControl(
				this));
		ControlManager.registerControl(CategoryLabels.IP, new IpAddressControl(
				this));
		ControlManager.registerControl(CategoryLabels.MAC,
				new MacAddressControl(this));
		ControlManager.registerControl(CategoryLabels.DEVICE,
				new DeviceControl(this));
	}
	...
	public void selectCategory(TreePath selectionPath) {
		...
		DefaultMutableTreeNode categoryNode = (DefaultMutableTreeNode) selectionPath
				.getLastPathComponent();
		if (categoryNode.getUserObject() instanceof CategoryPanel) {
			selectedCategory = (CategoryPanel) categoryNode.getUserObject();
		}
		if (selectedCategory != null) {
			// a new instance of this panel is required, so it can be added more
			// than one time to a swing component
			CategoryPanel categoryPanel = selectedCategory.newInstance();
			categoryPanel
					.setCharacteristicHandler(new CharacteristicSelectionHandler(
							this, selectedCategory.getCategory()));
			mMetalyzerPanel.addTab(categoryPanel.getCategory(), categoryPanel);
			fillAnalysePanel(categoryPanel.getCategory(),
					categoryPanel.getSelectedCharacteristic());
		} else {
	...
	/**
	 * Updates the {@link TimeSelectionView} of the given {@link AnalysePanel}
	 * and the panel itself.
	 * 
	 * @param analysePanel
	 * @param isLive
	 *            if true the given {@link AnalysePanel} will be updated
	 * */
	public void updateAnalysePanels(AnalysePanel analysePanel, boolean isLive) {
		TimeSelectionView timeSelectionView = analysePanel
				.getTimeSelectionView();
		timeSelectionView.setTimestampList(getTimestampList());
		if (isLive) {
			log.debug(analysePanel.getName() + " time live: "
					+ getLastTimestamp());
			...
			if (timeSelectionView.getSelectionMode() == TimeSelection.Timestamp) {
				log.debug("time update: "
						+ timeSelectionView.getTimeStampValue());
			} else if (timeSelectionView.getSelectionMode() == TimeSelection.Intervall) {
				log.debug("time update from: "
						+ timeSelectionView.getTimeStartValue() + " to: "
						+ timeSelectionView.getTimeEndValue());
			}
		}
	}

	/**
	 * Updates the given {@link AnalysePanel} with given timestamp intervall t1
	 * and t2
	 * 
	 * @param analaysePanel
	 *            the panel that should be updated e.g. {@link StatisticPanel}
	 *            or a {@link SemanticPanel}
	 * @param t1
	 *            a unix timestamp, and the first timestamp of the time
	 *            intervall
	 * @param t2
	 *            a unix timestamp, and the last timestamp of the time intervall
	 * */
	public void updateAnalysePanels(AnalysePanel analysePanel, long t1, long t2) {
		...
		if (analysePanel instanceof StatisticPanel) {
			...
			ControlManager.getControl(category).updateControl(mStatisticPanel,
					t1, t2);
		}
		if (analysePanel instanceof SemanticPanel) {
			mSemanticPanel = (SemanticPanel) analysePanel;
			ControlManager.getControl(category).updateControl(mSemanticPanel,
					t1, t2);
		}
	}
	 ...
	 * 
	 * @param analaysePanel
	 *            the panel that should be updated e.g. {@link UserPanel}
	 * @param item
	 *            e.g. the name of a user or an ip-address
	 * */
	...
	/**
	 * Updates the given {@link AnalysePanel} with given statisticPanel with a
	 * chart to adjust the scroll area that is modified by the
	 * {@link ChartAdjustmentHandler}
	 * 
	 * @param analaysePanel
	 *            the panel that should be updated e.g. {@link StatisticPanel}
	 * @param lowerBoundary
	 *            the minimum value that is allowed for the chart scrollbar,
	 *            depends on the scroll range
	 * @param upperBoundary
	 *            the maximum value that is allowed for the chart scrollbar,
	 *            depends on the scroll range
	 * */
	public void updateAnalysePanels(AnalysePanel analaysePanel,
		...
		if (analaysePanel instanceof StatisticPanel) {
			...
			if (control instanceof StatisticControl) {
				...
				statisticControl.updateChartScrollArea(mStatisticPanel,
	 ...
	 * 
	 * @param category
	 *            the category of the analysePanel that will be filled
	 * @param characteristic
	 *            the characteristic that is currently selected
	 * */
	public void fillAnalysePanel(String category, String characteristic) {
		...
		if (analysePanel instanceof StatisticPanel) {
			...
			ControlManager.getControl(category).fillControl(statisticPanel, t1,
					t2);
		} else {
			// Semantic
			SemanticPanel semanticPanel = (SemanticPanel) analysePanel;
			this.addObserver(semanticPanel);
			ControlManager.getControl(category).fillControl(semanticPanel, t1,
					t2);
		}
	}
