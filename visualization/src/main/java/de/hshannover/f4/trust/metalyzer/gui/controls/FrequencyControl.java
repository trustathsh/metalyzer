	...
	public void updateControl(AnalysePanel panel, long t1, long t2) {
		...
		try {
			if (characteristic.equals(CharacteristicLabels.FREQUENCY_IDENTIFER)) {
				...
				case Intervall:
					relativeFrequency = mConnection.getFrequencyFromTo(t1, t2,
							FrequencyType.IDENTIFIER,
							FrequencySelection.RELATIVE_FREQUENCY);
					absoluteFrequency = mConnection.getFrequencyFromTo(t1, t2,
							FrequencyType.IDENTIFIER,
							FrequencySelection.ABSOLUTE_FREQUENCY);
					break;
				case Live:
					relativeFrequency = mConnection.getFrequencyCurrent(
							FrequencyType.IDENTIFIER,
							FrequencySelection.RELATIVE_FREQUENCY);
					absoluteFrequency = mConnection.getFrequencyCurrent(
							FrequencyType.IDENTIFIER,
							FrequencySelection.ABSOLUTE_FREQUENCY);
					break;
				case Timestamp:
					relativeFrequency = mConnection.getFrequencyTimestamp(t2,
							FrequencyType.IDENTIFIER,
							FrequencySelection.RELATIVE_FREQUENCY);
					absoluteFrequency = mConnection.getFrequencyTimestamp(t2,
							FrequencyType.IDENTIFIER,
							FrequencySelection.ABSOLUTE_FREQUENCY);
					break;
				default:
