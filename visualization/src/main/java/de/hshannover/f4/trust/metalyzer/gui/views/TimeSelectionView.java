...
public class TimeSelectionView extends View {
	...
	private void createTimeIntervallSelection(Set<Long> timestamps,
			String dateFormat) {
		...
		// Catch these exceptions to avoid the termination of the polling
		// service
		try {
			mTimeStartSelector = new JSpinner(new SpinnerListModel(mStart));
		} catch (IllegalArgumentException e) {
			log.error(e.getMessage());
			mTimeStartSelector = new JSpinner(new SpinnerListModel(
					UtilHelper.createDummyList()));
		} finally {
			mTimeStartSelector.setEditor(new TimeListEditor(mTimeStartSelector,
					dateFormat));
		...
		try {
			...
			log.error(e.getMessage());
			mTimeEndSelector = new JSpinner(new SpinnerListModel(
					UtilHelper.createDummyList()));
		} finally {
			mTimeEndSelector.setEditor(new TimeListEditor(mTimeEndSelector,
					dateFormat));
			mTimeEndSelector.setEnabled(mTimeIntervall.isSelected());
		}

		mIntervallGroup = new RowGroup();
		layout.row().group(mIntervallGroup).grid().indent(70)
				.add(mTimeIntervall).add(mTimeStartSelector)
				.add(mTimeEndSelector).empty(3);
	}

	private void createTimestampSelection(Set<Long> timestamps,
			String dateFormat) {
		...
		try {
			...
			log.error(e.getMessage());
			mTimeStampSelector = new JSpinner(new SpinnerListModel(
					UtilHelper.createDummyList()));
		} finally {
			mTimeStampSelector.setEditor(new TimeListEditor(mTimeStampSelector,
					dateFormat));
			mTimeStampSelector.setEnabled(mTimeStamp.isSelected());
		}

		mStampGroup = new RowGroup();
		layout.row().group(mStampGroup).grid().indent(70).add(mTimeStamp)
				.add(mTimeStampSelector).empty(4);
	}
	 ...
	 * 
	 * @param selection
	 *            the mode with time selection view is initialized.<br>
	 ...
	 * 
	 * @param timestamps
	 *            reinitializes the {@link JSpinner} of the TimeSelectionView
	 * */
	public void setTimestampList(SortedSet<Long> timestamps) {
		if ((mTimeStartSelector != null) && (mTimeEndSelector != null)) {
			...
			SpinnerListModel mTimeStampModel = (SpinnerListModel) mTimeStampSelector
					.getModel();
			...
			SpinnerListModel mTimeStartModel = (SpinnerListModel) mTimeStartSelector
					.getModel();
			try {
			...
			SpinnerListModel mTimeEndModel = (SpinnerListModel) mTimeEndSelector
					.getModel();
			try {
	...
	/**
	 * Adds a {@link ChangeListener} to the timestamp intervall selector
	 * {@link JSpinner}
	 * 
	 * @param listener
	 *            the spinner changelisteners
	...
	/**
	 * Returns an instance of the timestamp intervall button
	 * {@link JRadioButton}
	 * 
	...
	/**
	 * Sets the time stamp value if the value exists in the current spinner
	 * list, otherwise the value will be irgnored,
	 * */
	...
	/**
	 * Sets the time start value if the value exists in the current spinner
	 * list, otherwise the value will be irgnored,
	 * */
	 ...
	 * 
	 * @param selection
	 *            keep this TimeSelector hides the rest
	 * */
